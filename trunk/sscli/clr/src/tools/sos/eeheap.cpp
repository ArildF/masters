...
void GCHeapInfo(gc_heap &heap, DWORD_PTR &total_size)
{
    ...
    for (n = 0; n <= heap.g_max_generation; n ++)
    {
        ...
        dprintf ("generation %d starts at 0x%p\n",
                 n, (ULONG64)(UINT_PTR)heap.generation_table[n].allocation_start);
    }
    ...
    while (dwAddrSeg != (DWORD_PTR)heap.generation_table[0].start_segment)
    {
        ...
        dwAddr = dwAddrSeg;
        segment.Fill (dwAddr);
        dprintf ("%p %p  %p 0x%p(%d)\n", (ULONG64)(UINT_PTR)dwAddrSeg,
                 (ULONG64)(UINT_PTR)segment.mem, (ULONG64)(UINT_PTR)segment.allocated,
                 (ULONG64)(UINT_PTR)(segment.allocated - segment.mem),
    ...
    dprintf ("%p %p  %p %p(%d)\n", (ULONG64)dwAddrSeg,
             (ULONG64)(UINT_PTR)segment.mem, (ULONG64)end,
                ...
                AllocInfo* pallocInfo)
{
    ...
    while(1)
    {
            ...
            && (flags.MT == 0 || flags.MT == mtAddr))
        {
            ...
            gStat->Add (dwAddrMethTable, (DWORD)s);
        }
...
DWORD_PTR LoaderHeapInfo (LoaderHeap *pLoaderHeap)
{
    ...
    while (1)
    {
        ...
        totalSize += curSize;
        dprintf ("%p(%x", (ULONG64)(UINT_PTR)heapBlock.pVirtualAddress,
                 heapBlock.dwVirtualSize);
