...
/*
 * This is the index in thread local storage of the per-thread context
 *  pointer, for use in PAL_wcstok. 
 * It is initialized and freed in PAL_Initialize() and
 * PAL_Terminate, respectively
 */
pthread_key_t wcstokKey = PTHREAD_KEYS_MAX;
...
PAL_wcstoulExit:
    free(s_nptr);
    LOGEXIT("wcstoul returning unsigned long %ld\n", res);
    return res;
}
