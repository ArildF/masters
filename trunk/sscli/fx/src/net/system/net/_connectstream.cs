...
namespace System.Net {
    ...
    internal class ConnectStream : Stream, IDisposable {
        ...
        private void SetWriteDone() {
            if (m_WriteDoneEvent!=null && BytesLeftToWrite <=0) {
                GlobalLog.Print("ConnectStream#" + ValidationHelper.HashString(this) + "::WaitWriteDone() setting WriteDoneEvent");
        ...
        internal void WaitWriteDone() {
            if (BytesLeftToWrite>0) {
                if (m_WriteDoneEvent==null) {
                    GlobalLog.Print("ConnectStream#" + ValidationHelper.HashString(this) + "::WaitWriteDone() creating WriteDoneEvent BytesLeftToWrite:" + BytesLeftToWrite.ToString());
        ...
        public ConnectStream(
            ...
            if (request.BufferHeaders) {
                m_BufferOnly = true;
                //
                // we need to save a reference to the request for when, later in the code,
                // the client closes the write stream. at that point we'll wake up the
                // request to send the headers and kick off resubmitting of buffered entity body
                //
                m_Request = request;
        ...
        private int ReadWithoutValidation([In, Out] byte[] buffer, int offset, int size) {
            ...
            IAsyncResult asyncResult = this.BeginReadWithoutValidation(buffer, offset, size, null, null);
            return this.EndReadWithoutValidation((NestedSingleAsyncResult)asyncResult);
        }
        ...
        internal void CloseInternal(bool internalCall) {
            ...
            if ( Interlocked.Exchange( ref m_ShutDown,  1) == 1 ) {
                GlobalLog.Leave("ConnectStream#" + ValidationHelper.HashString(this) + "::CloseInternal", "ShutDown");
                return;
            }
        ...
        internal virtual ConnectStream MakeMemoryStream() {
            ...
            if (!this.Eof) {
                ...
                try {
                    while (! ErrorInStream && m_ShutDown != 1 && (BytesTransferred = Read(Buffer, 0, 1024)) > 0) {
        ...
        private bool DrainSocket() {
            ...
            if (m_ReadBytes > m_MaxDrainBytes) {
                GlobalLog.Print("ConnectStream#" + ValidationHelper.HashString(this) + "::DrainSocket() m_ReadBytes:" + m_ReadBytes.ToString() + " too large, calling Abort()");
                m_Connection.Abort();
