...
const GUID ILDB_VERSION_GUID_FSR = 
    { VAL32(0xCB2F6723), VAL16(0xAB3A), VAL16(0x11d2), {0x9C, 0x40, 0x00, 0xC0, 0x4F, 0xA3, 0x0A, 0x3E} };

const GUID ILDB_VERSION_GUID = 
    { VAL32(0x9e02e5b6), VAL16(0x8aef), VAL16(0x4d06), { 0x82, 0xe8, 0xe, 0x9b, 0x45, 0x49, 0x97, 0x16 } };
...
HRESULT DumpIldbFile(char *szIldbFileName)
{
    ...
    {
        if (memcmp(pVersionInfo, &ILDB_VERSION_GUID_FSR, sizeof(GUID)) == 0)
        {
...
HRESULT GetMetaFromFile(LPCWSTR wcsFileName, IMetaDataImport **ppMetaImport)
{
    ...
    pNtHeader = Cor_RtlImageNtHeader(g_hMod);
    if (pNtHeader == NULL) {
        IfFailGo(HrFromWin32(ERROR_BAD_FORMAT));
    }

    pICH = Cor_RtlImageCorHeader(pNtHeader,
                                 g_hMod, 
                                 dwLength);
    if (pICH == NULL)
    {
