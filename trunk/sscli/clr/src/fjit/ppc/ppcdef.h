...
_ASSERTE( BO <=20 && BI < 32 && (TAKEN == 0 || TAKEN == 1 ) && (LK == 0 || LK == 1 ));\
cmdDWord(expOr4( W_PPC_OPCODE(19), W_REGD((BO & (TAKEN | 30))), W_REGA(BI), W_VALUE(1056, 30, 16 )) | LK );\
DBG_CG(printf(": %x \n", *(((unsigned *)outPtr)-1) ); ) }
...
{                                                                                           \
  ...
  if (adj)                                                                                  \
  {                                                                                         \
    ppc_address = (((__int32)absAddress - (__int32)outPtr) >> 2);                           \
    if ( !FitsInI2((ppc_address << 2)) && ( cond & 0xF ) != ppc_CR_NOTUSED )                \
      { ppc_address = (((__int32)absAddress - (__int32)storedStartIP)); largeBranch = true; }\
  }                                                                                         \
  else                                                                                      \
  {                                                                                         \
    ppc_address = ((unsigned)absAddress);                                                   \
...
{                                                                                           \
  ...
  if ( R_PPC_OPCODE(*ppc_iStart) == 16 )                                                    \
    *iStart = ( ((*ppc_iStart) & 0xFFFF0000) | W_VALUE(ppc_address, 29, 16 ) | ((*ppc_iStart) & 3) );   \
  else if ( R_PPC_OPCODE(*ppc_iStart)  == 18 )                                              \
    *iStart = ( ((*ppc_iStart) & 0xF8000000) | W_VALUE(ppc_address, 29, 6 ) | ((*ppc_iStart) & 3) );    \
...
{                                                           \
   ...
   ppc_move_constant(R12, SWITCH_helper );                  \
   ppc_mtspr(R12, PPC_MOVE_LR);                             \
   ppc_bclr(BO_8, 0, SAVE_RET_ADDR, PPC_PREDICT_TAKEN);     \
}
