//------------------------------------------------------------------------------
// <copyright file="WebHeaders.cs" company="Microsoft">
//     
...
namespace System.Net {
    ...
    public class WebHeaderCollection : NameValueCollection, ISerializable {
        ...
        protected void AddWithoutValidate(string headerName, string headerValue) {
            ...
            GlobalLog.Print("WebHeaderCollection::AddWithoutValidate: key:[" + headerName + "], value:[" + headerValue + "]");
        ...
        internal DataParseStatus ParseHeaders(byte[] buffer, int size, ref int unparsed) {
            ...
            int index = unparsed;
            bool spaceAfterLf;
            bool isUTF8HeaderName;
            bool isUTF8HeaderValue;
            string headerMultiLineValue;
            string headerName;
            string headerValue;

            GlobalLog.Print("WebHeaderCollection::ParseHeaders(): size:" + size.ToString() + ", unparsed:" + unparsed.ToString() + " buffer:[" + Encoding.ASCII.GetString(buffer, unparsed, Math.Min(256, size-unparsed)) + "]");
            ...
            for (;;) {
                ...
                spaceAfterLf = false;
                isUTF8HeaderName = false;
                isUTF8HeaderValue = false;
                ...
                while (index<size && (buffer[index]!=':' && buffer[index]!='\n')) {
                    if (buffer[index]>' ') {
                        ...
                        isUTF8HeaderName = isUTF8HeaderName || buffer[index]>0x7f;
                    }
                    index++;
                }
                if (index==size) {
...
startOfValue:
                ...
                while (index<size && buffer[index]!='\n') {
                    if (buffer[index]>' ') {
                        ...
                        isUTF8HeaderValue = isUTF8HeaderValue || buffer[index]>0x7f;
                    }
                    index++;
                }
                if (index==size) {
...
addHeader:
                if (headerValueStartOffset>=0 && headerValueStartOffset>headerNameEndOffset && headerValueEndOffset>=headerValueStartOffset) {
                    //
                    // Encoding.ASCII is the fastest way to build the UNICODE string off the byte[]
                    //
                    if (isUTF8HeaderValue) {
                        headerValue = Encoding.UTF8.GetString(buffer, headerValueStartOffset, headerValueEndOffset-headerValueStartOffset+1);
                    }
                    else {
                        headerValue = Encoding.ASCII.GetString(buffer, headerValueStartOffset, headerValueEndOffset-headerValueStartOffset+1);
                    }
                }
                ...
                if (headerNameStartOffset>=0 && headerNameEndOffset>=headerNameStartOffset) {
                    ...
                    //
                    if (isUTF8HeaderName) {
                        headerName = Encoding.UTF8.GetString(buffer, headerNameStartOffset, headerNameEndOffset-headerNameStartOffset+1);
                    }
                    else {
                        headerName = Encoding.ASCII.GetString(buffer, headerNameStartOffset, headerNameEndOffset-headerNameStartOffset+1);
                    }
                }
        ...
        : base(CaseInsensitiveAscii.StaticInstance, CaseInsensitiveAscii.StaticInstance) {
            ...
            for (int i = 0; i < count; i++) {
                base.Add(
                    ...
                    serializationInfo.GetString((i+count).ToString()) );
            }
            return;
        }
    ...
    internal class CaseInsensitiveAscii : IHashCodeProvider, IComparer {
        ...
        public int GetHashCode(object myObject) {
            ...
            myHashCode ^= AsciiToLower[myString[0]]<<24 ^ AsciiToLower[myString[myHashCode-1]]<<16;
