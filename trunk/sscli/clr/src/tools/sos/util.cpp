...
ModuleInfo moduleInfo[MSCOREND] = {
    {0,FALSE},
    {0,FALSE}
};
...
BOOL CheckEEDll ()
{
    ...
    if (!MscoreeDone) {
        ...
        // Do we have sscoree.dll loaded?
#ifdef PLATFORM_UNIX
        if (moduleInfo[MSCOREE].baseAddr == 0) {
	    // Get the module handle via LoadLibrary, but immediately unload it
	    // so the refcount doesn't remain.
            HMODULE hMod = LoadLibrary(MAKEDLLNAME("sscoree"));
	    moduleInfo[MSCOREE].baseAddr = (ULONG64)(UINT_PTR)hMod;
	    FreeLibrary(hMod);
	}
#else //!PLATFORM_UNIX
        if (moduleInfo[MSCOREE].baseAddr == 0)
                g_ExtSymbols->GetModuleByModuleName ("sscoree",0,NULL,
        ...
        if (moduleInfo[MSCOREE].baseAddr != 0 && moduleInfo[MSCOREE].hasPdb == FALSE)
        {
            ...
            {
                g_ExtSymbols->Reload("/f sscoree.dll");
                g_ExtSymbols->GetModuleParameters (1, &moduleInfo[MSCOREE].baseAddr, 0, &Params);
            }
        ...
        if (moduleInfo[MSCOREE].baseAddr != 0 && moduleInfo[MSCOREE].hasPdb == FALSE)
            dprintf ("PDB symbol for sscoree.dll not loaded\n");
#endif //!PLATFORM_UNIX
    }
...
{
    static ULONG platform = 0xffffffff;
...
* Routine Description:                                                 *
*                                                                      *
*    This function is called to find out if a dll is a retail build    *  
*                                                                      *
\**********************************************************************/
BOOL IsRetailBuild (size_t base)
{
    ...
    if (g_ExtData->ReadVirtual(base + VAL32(DosHeader.e_lfanew), &Header32, sizeof(Header32), NULL) != S_OK)
        return FALSE;
    ...
    size_t debugDirAddr = base + VAL32(Header32.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress);
    size_t nSize = VAL32(Header32.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size);
    IMAGE_DEBUG_DIRECTORY debugDir;
    size_t nbytes = 0;
    while (nbytes < nSize) {
        if (g_ExtData->ReadVirtual(debugDirAddr+nbytes, &debugDir, sizeof(debugDir), NULL) != S_OK)
            return FALSE;
        if (debugDir.Type == VAL32(0xA)) {
            return TRUE;
        }
                    ...
                    DWORD_PTR &gcinfoAddr)
{
    ...
    case EJIT: {
        ...
        DWORD_PTR vJMIT;
        DWORD dwJMITBlockSize;
        
        // static for pJMIT moved to file static
        move (vJMIT, pJMIT);
        
        dwJMITBlockSize = OSPageSize(); // size of individual blocks of JMITs that are chained together
        while (vJMIT)
        {
            if (IsInterrupt())
                return;
            if (IP >= vJMIT && IP < vJMIT + dwJMITBlockSize)
            {
                DWORD_PTR u1 = IP + 8;
                DWORD_PTR MD;
                move (u1, u1);
...
void DisplayDataMember (FieldDesc* pFD, DWORD_PTR dwAddr, BOOL fAlign=TRUE)
{
    if (dwAddr > 0)
    {
        ...
        if (gElementTypeInfo[pFD->m_type] != (DWORD_PTR)-1)
        {
            union Value
            {
                char ch;
...
void DisplayFields (EEClass *pEECls, DWORD_PTR dwStartAddr, BOOL bFirst)
{
           ...
           || numStaticFields < pEECls->m_wNumStaticFields)
    {
        ...
        dprintf ("%p %8x %8x ",
                 ((ULONG64)(UINT_PTR)vFieldDesc.m_pMTOfEnclosingClass & ~0x3),
...
void FileNameForModule (Module *pModule, WCHAR *fileName)
{
    ...
    if (vPEFile.m_wszSourceFile[0] != L'\0') {
        MatchDllsName (vPEFile.m_wszSourceFile, fileName, (ULONG64)(UINT_PTR)vPEFile.m_base);
    }
    else
...
{
    dprintf ("ClassLoader: %p\n", (ULONG64)(UINT_PTR)pAssembly->m_pClassLoader);
...
{
    dprintf ("LowFrequencyHeap: %p\n", (ULONG64)(UINT_PTR)pDomain->m_pLowFrequencyHeap);
    dprintf ("HighFrequencyHeap: %p\n", (ULONG64)(UINT_PTR)pDomain->m_pHighFrequencyHeap);
    dprintf ("StubHeap: %p\n", (ULONG64)(UINT_PTR)pDomain->m_pStubHeap);
    dprintf ("Name: ");
...
void SharedDomainInfo (DWORD_PTR DomainAddr)
{
    ...
    dprintf ("LowFrequencyHeap: %p\n", (ULONG64)(UINT_PTR)v_SharedDomain.m_pLowFrequencyHeap);
    dprintf ("HighFrequencyHeap: %p\n", (ULONG64)(UINT_PTR)v_SharedDomain.m_pHighFrequencyHeap);
    dprintf ("StubHeap: %p\n", (ULONG64)(UINT_PTR)v_SharedDomain.m_pStubHeap);
    ...
    while (nBucket > 0) {
        ...
        for (int i = 0; i < 4; i ++) {
            ...
            if (dwAssemblyAddr) {
                // From vm/hash.h: Buckets store values right-shifted by 1
                dwAssemblyAddr <<= 1;
...
void NameForMD (MethodDesc *pMD, WCHAR *mdName)
{
    ...
    if (CallStatus)
    {
        if (IsDebugBuildEE())
        {
            ...
            PrintString (EEClassAddr, FALSE, (DWORD_PTR)-1, mdName);
            wcscat (mdName, L".");
            static WCHAR name[2048];
            name[0] = L'\0';
            PrintString ((DWORD_PTR)pMD->m_pszDebugMethodName,
                         FALSE, (DWORD_PTR)-1, name);
            wcscat (mdName, name);
        }
        else
        {
...
BOOL IsSameModuleName (const char *str1, const char *str2)
{
    ...
    if ((ptr1 < str1) || (ptr1 >= str1 && *ptr1 != '\\' && *ptr1 != '/' && *ptr1 != ':'))
        return FALSE;
    return TRUE;
}
...
void DumpMDInfo(DWORD_PTR dwStartAddr, BOOL fStackTraceFormat)
{
    ...
    if (!fStackTraceFormat)
    {
        ...
        dprintf("Flags : %x\r\n",pMD->m_wFlags);
        if ((pMD->m_CodeOrIL & METHOD_IS_IL_FLAG) == METHOD_IS_IL_FLAG)
        {
            dprintf("IL RVA : %p\r\n",(DWORD64)(ULONG_PTR)(pMD->m_CodeOrIL & ~METHOD_IS_IL_FLAG));
        }
        else
        {
            dprintf("Method VA : %p\r\n",(DWORD64)(ULONG_PTR)pMD->m_CodeOrIL);
        }
    }
    else
        dprintf ("%S\n", (WCHAR*)fullname.Ptr());
...
{
    static ULONG bFixBadSymbols=0xffffffff;
    if (bFixBadSymbols != 0xffffffff) {
...
size_t ObjectSize (DWORD_PTR obj)
{
    ...
    if (vMT.GetComponentSize()> 0)
    {
        DWORD_PTR pComp = obj + 4;
        DWORD_PTR numComp;
        moveN (numComp, pComp);
        size += vMT.GetComponentSize()*numComp;
    }
    size = Align (size);
    return size;
}
...
void StringObjectContent (size_t obj, BOOL fLiteral, const int length)
{
    ...
    DWORD_PTR mask;
    move (count, dwAddr);
    mask = ((DWORD_PTR)-1) >> 4; // 0x0fffffff on 32-bits
    count &= mask;
