    ...
    internal class RemotingXmlConfigFileParser
    {
        ...
        private static void ProcessDebugNode(ConfigNode node, RemotingXmlConfigFileData configData)
        {
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                default: ReportUnknownAttributeError(node, key, configData); break;                
                } // switch
            } // foreach

            VerifyThatThereAreNoChildren(node, configData);
        } // ProcessDebugNode
        ...
        private static void ProcessApplicationNode(ConfigNode node, RemotingXmlConfigFileData configData)
        {
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                if (key.Equals("name"))
                    ...
                    ReportUnknownAttributeError(node, key, configData);
            }
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                default: ReportUnknownChildError(node, childNode, configData); break;
                } // switch
            } // foreach
        ...
        {
            VerifyThatThereAreNoAttributes(node, configData);
        ...
        private static void ProcessChannelsNode(ConfigNode node, RemotingXmlConfigFileData configData)
        {
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    ReportUnknownChildError(node, childNode, configData); break;
                }
            } // foreach
        } // ProcessInteropNode
        ...
        // call this on any nodes that shouldn't have any xml attributes
        private static void VerifyThatThereAreNoAttributes(ConfigNode node,
                                                           RemotingXmlConfigFileData configData)
        {
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ReportUnknownAttributeError(node, entry.Key.ToString(), configData);
            }
        } // VerifyThatThereAreNoAttributes


        // call this on any nodes that shouldn't have any xml subnodes
        private static void VerifyThatThereAreNoChildren(ConfigNode node,
                                                         RemotingXmlConfigFileData configData)
        {
            foreach (ConfigNode childNode in node.Children)
            {
                ReportUnknownChildError(node, childNode, configData);                 
            }
        } // VerifyThatThereAreNoAttributes
