...
public class Co5761ctor_i_fa_b_i
{
    ...
    public bool runTest()
    {
        ...
        try
        {
            ...
            fs2.Flush();
            fs2.Position = 0;
            fs2 = new FileStream(fs2.Handle, FileAccess.Read, true, 8);
            bReadArr = new Byte[5];
            int read = fs2.Read(bReadArr, 0, 5);
            int i = 65;
            ...
            try
            {
                fs = new FileStream(fs.Handle, FileAccess.ReadWrite, true, Int32.MaxValue/100);
                Byte[] bArr = new Byte[100*10];
                for(int ii = 0 ; ii < bArr.Length ;ii++)
                    bArr[i] = (Byte)ii;
                IAsyncResult iar = fs.BeginWrite(bArr, 0, bArr.Length, null, null);
                fs.EndWrite(iar);
                fs.Close();
            } 
            catch( Exception e )
