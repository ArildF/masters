...
CAssemblyCacheItem::~CAssemblyCacheItem()
{
    ...
    if (_hrError != S_OK && _szDir[0])
    {
#ifdef DEBUG
        HRESULT hr =
#endif
        RemoveDirectoryAndChildren (_szDir);
#ifdef DEBUG
        if(hr != S_OK)
        {
            WCHAR szMsgBuf[MAX_PATH*2 + 1];
            wnsprintf( szMsgBuf, MAX_PATH*2, L"     CacheItem~:  RemoveDirectoryAndChildren FAILED for <%s>, hr = <%x>, _hrError = <%x>\r\n", 
                                            _szDir, hr, _hrError);
...
#ifdef DEBUG
    if((hr != S_OK) && (hr != DB_E_DUPLICATE))
    {
        WCHAR szMsgBuf[MAX_PATH*2 + 1];
        wnsprintf( szMsgBuf, MAX_PATH*2, L"Init FAILED with hr = <%x>\r\n", hr);
        WriteToLogFile(szMsgBuf);
    }
#endif  // DEBUG
            goto exit;
...
#ifdef DEBUG
            WCHAR szMsgBuf[MAX_PATH*2 + 1];
            wnsprintf( szMsgBuf, MAX_PATH*2, L"   Could not get lock for <%s>, hr = <%x> KB\r\n", 
                            pszManifestPath, hr);
            WriteToLogFile(szMsgBuf);
#endif  // DEBUG
        }
        ...
        /* [in, optional] */ ULARGE_INTEGER *puliMaxSize) // Max size of the Stream.
{
    ...
    *ppStream = NULL;
    ...
    switch (dwFormat)
    {
        ...
        {
            if((_dwCacheFlags & ASM_CACHE_DOWNLOAD) && (_pszUrl) 
                )
            {
...
HRESULT CAssemblyCacheItem::VerifyDuplicate(DWORD dwVerifyFlags, CTransCache *pTC)
{
    ...
    if(!pTC->_pInfo->pwzCodebaseURL || StrCmpI(pTC->_pInfo->pwzCodebaseURL, _pszUrl))
    {
        hr = E_FAIL;
        goto exit;
    }
...
#define MAX_DIR_LEN  (5)
    ...
    {
        if(StrCmpNI(wzManifestFileName, szAsmTextName, lstrlenW(szAsmTextName)))
        {
    ...
    /* [out, optional] */ ULONG *pulDisposition)
{
    ...
    if (CCache::IsStronglyNamed(_pName) && !_pbCustom)
    {
        BOOL fWasVerified;
    ...
    if(!_pTransCache) // this asm is being added first time and not incremental download
    {
        ...
        _pName->SetProperty(ASM_NAME_CODEBASE_URL, (LPWSTR) _pszUrl, 
            _pszUrl ? (DWORD)((lstrlen(_pszUrl) + 1) * sizeof(WCHAR)) : 0);
