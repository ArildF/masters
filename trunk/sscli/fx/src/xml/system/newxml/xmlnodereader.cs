...
namespace System.Xml
{
    ...
    public class XmlNodeReader: XmlReader
    {
        ...
        public override string ReadString() {
            if ( !IsInReadingStates() )
                return string.Empty;
            if( ( nodeType == XmlNodeType.Attribute ) || ( readerNav.IsOnAttrVal ) ) {
                XmlElement ownerElem = readerNav.OwnerElementNode as XmlElement;
                if( ownerElem == null || ownerElem.IsEmpty )
                    return string.Empty;
                if( ! Read() )
                    return string.Empty;
            }
            //For EntityReference node, ReadString should return String.Empty unless ResolveEntity is called.
            else if ( ( nodeType == XmlNodeType.Element && !IsEmptyElement )|| ( nodeType == XmlNodeType.EntityReference && bResolveEntity )  ) {
                if (! Read())
                    throw new InvalidOperationException(Res.GetString(Res.Xml_InvalidOperation));
            }

            //nav doesn't have to be on element node
        ...
        /// <include file='doc\XmlNodeReader.uex' path='docs/doc[@for="XmlNodeReader.ReadInnerXml"]/*' />
        /// <devdoc>
        ///    <para>Reads all the content (including markup) as a string.</para>
        /// </devdoc>
        public override String ReadInnerXml() {
            if ( !fEOF && nodeType != XmlNodeType.None && nodeType != XmlNodeType.EndElement ) {
                switch ( nodeType ) {
                    case XmlNodeType.Element: {
                        String retstr = readerNav.InnerXml;
                        Skip();
                        return retstr;
                    }
                    case XmlNodeType.Attribute:
                        return readerNav.InnerXml;
                }
            }
            Skip();
            return String.Empty;
        ...
        //  attributes of DocumentType and XmlDeclaration nodes
        /// <include file='doc\XmlNodeReader.uex' path='docs/doc[@for="XmlNodeReader.ReadOuterXml"]/*' />
        /// <devdoc>
        ...
        public override string ReadOuterXml() {
            if ( !fEOF && nodeType != XmlNodeType.None && nodeType != XmlNodeType.EndElement ) {
                switch ( nodeType ) {
                    case XmlNodeType.Element: {
                        String retstr = readerNav.OuterXml;
                        Skip();
                        return retstr;
                    }
                    case XmlNodeType.Attribute:
                        return readerNav.OuterXml;
                }
            }
            Skip();
            return String.Empty;
