...
// constructors
        ...
        public Uri(string uriString, bool dontEscape) {
            ...
            m_AbsoluteUri = NonPathPart +
                            m_Path +
                            ((m_Query.Length > 0) ? m_Query : m_Fragment);
...
// properties
        ...
        internal string Extra {
            get {
                return (m_Fragment.Length > 0) ? m_Fragment : m_Query;
            }
        }

        internal string ExtraDelimiter {
            get {
                if (m_Fragment.Length > 0) {
                    return "#";
                }
                else if (m_Query.Length > 0) {
                    return "?";
                }
                return String.Empty;
            }
...
// methods
        ...
        //      Unix only:
        //          /path
        //          file:/path (or more leading slashes)
        //
        // Inputs:
        //  <member>    m_Path
        ...
        protected virtual void Parse() {
            ...
            if (index > 1) {
                ...
                //
                // assume file - either UNC (Windows only) or local path
                //
...
#if PLATFORM_UNIX
            // On Unix, don't remove leading slashes for file URLs
            if (m_Scheme != UriSchemeFile) {
#endif  // PLATFORM_UNIX
            ...
            // with <letter> (':' | '|') ('/' | '\\') on Windows or ('/' | '\\')
            // on Unix then this is an error. UNC file paths are not
            // permitted on Unix.
            // Legal file paths must be rooted local drive paths.
            // Relative paths will be thrown out
            //

#if PLATFORM_UNIX
            bool looksLikeAbsolutePath = (uriString.Length - index >= 1
                                          && ((uriString[index] == '/' || (uriString[index] == '\\'))));
#else   // PLATFORM_UNIX
            bool looksLikeAbsolutePath = (uriString.Length - index >= 3)
                                          && Char.IsLetter(uriString[index])
                                          && ((uriString[index + 1] == ':') || (uriString[index + 1] == '|'))
                                          && ((uriString[index + 2] == '/') || (uriString[index + 2] == '\\'));
#endif  // PLATFORM_UNIX
            if (m_IsFile && !looksLikeAbsolutePath) {
#if !PLATFORM_UNIX
                if (nSlashes >= 2) {
                    m_IsUnc = true;
                    m_IsFileReally = true;
            ...
            //
            // 4. parse fragment or query if URI didn't have a scheme (it is a
            // local or UNC path)
            //
                ...
                if (fi != -1) {
                    ...
                    if (len > 0) {
                        ...
                        if (!m_AlreadyEscaped) {
                            ...
                            //
                            // N.B. Although m_AlreadyEscaped is true, the fragment
                            // may or may not be escaped. If it isn't and we set
                            // m_FragmentEscaped to true then we will always run
                            // Unescape() on the fragment in DisplayFragment et al.
                            // Since the fragment is probably only a short string,
                            // this is not a big hit. We could either test to
                            // ensure the correct value of m_FragmentEscaped, or
                            // keep an unescaped copy. However, I really don't
                            // think it will be a big problem. (Same goes for
                            // query below)
                            //
                    ...
                    int len = uriString.Length - qi;
                    string s = "";

                    if (len > 0) {
                        s = uriString.Substring(qi, len);
                        if (!m_AlreadyEscaped) {
        ...
        public override string ToString() {
            if (m_String == null) {
                m_String = DisplayNameNoExtra + ((m_Query.Length > 0)? DisplayQuery: 
                                                ((m_Fragment.Length > 0) ? DisplayFragment:
                                                 String.Empty));
            }
            return m_String;
        }
