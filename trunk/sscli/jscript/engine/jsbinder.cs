    ...
    internal sealed class JSBinder : Binder{
      ...
      public override FieldInfo BindToField(BindingFlags bindAttr, FieldInfo[] match, Object value, CultureInfo locale){
        ...
        for (int i = 0, j = match.Length; i < j; i++){
          ...
          int distance = TypeDistance(field.FieldType, vtype);
          if (distance < minval){
            minval = distance;
            bestField = field;
            dupCount = 0;
      ...
      public override MethodBase BindToMethod(BindingFlags bindingAttr, MethodBase[] match, ref Object[] args, ParameterModifier[] modifiers, 
        ...
        state = null;
        return JSBinder.SelectMethodBase(match, ref args, modifiers, namedParameters);
      }
      ...
      internal static MemberInfo[] GetDefaultMembers(IReflect ir){
        ...
        if (ir is JSObject) return (typeof(ScriptObject)).GetDefaultMembers();
        return null;
      }
      
      internal static MemberInfo[] GetDefaultMembers(Type t){
        while (t != Typeob.Object && t != null){
      ...
      internal static MemberInfo Select(MemberInfo[] match, int matches, IReflect[] argIRs, MemberTypes memberType){
        ...
        int j = JSBinder.SelectBest(match, matches, argIRs, fparams, null, candidates, argIRs.Length);
        if (j < 0) return null;
        return match[j];
      }
      
      internal static MemberInfo Select(MemberInfo[] match, int matches, ref Object[] args, String[] namedParameters, MemberTypes memberType){
        Debug.Assert(matches > 1);
      ...
      private static int SelectBest(MemberInfo[] match, int matches, IReflect[] argIRs, ParameterInfo[][] fparams, Object[][] aparams, int candidates, int parameters){ 
        Debug.Assert(matches > 1);
        ...
        for (int i = 0; i < matches; i++){
          ...
          if (fpars != null){
            ...
            // Eliminate the candidates which have fewer formal parameters than actuals ( and for which
            // the last formal parameter is not a param-array). We do not consider these in overload
            // resolution.
            if (actuals > m && (m == 0 || !fpars[m-1].IsDefined(Typeob.ParamArrayAttribute, false))){
              fparams[i] = null;
              candidates--;
            ...
            for (int j = parameters; j < m; j++){
              ...
              if (j == m-1 && fpar.IsDefined(Typeob.ParamArrayAttribute, false))
                break;
              Object dv = fpar.DefaultValue;
        ...
        for (int p = 0; candidates > 1; p++){
          ...
          for (int i = 0; i < matches; i++){
            ...
            if (fpars != null){ //match[i] is a candidate
              ...
              IReflect fIR = Typeob.Missing;
              if (m > 0 && p >= m-1 && fpars[m-1].IsDefined(Typeob.ParamArrayAttribute, false) && 
              !(aIR is TypedArray || aIR == Typeob.ArrayObject || (aIR is Type && ((Type)aIR).IsArray))){
              ...
              int distance = TypeDistance(fIR, aIR) + penalty[i]; 
              if (distance == minval){
              ...
              if (distance > minval){
                ...
                if (p > m-1 && aIR == Typeob.Missing && fpars[m-1].IsDefined(Typeob.ParamArrayAttribute, false)) continue;
                eliminated[i] = true;
                continue;
              }
      ...
      internal static ConstructorInfo SelectConstructor(MemberInfo[] match, ref Object[] args, String[] namedParameters){
        ...
        return (ConstructorInfo)JSBinder.Select(match, n, ref args, namedParameters, MemberTypes.Constructor);
      }  
      ...
      internal static ConstructorInfo SelectConstructor(MemberInfo[] match, IReflect[] argIRs){
        ...
        if (n == 0) return null;
        if (n == 1) return match[0] as ConstructorInfo;
        return (ConstructorInfo)JSBinder.Select(match, n, argIRs, MemberTypes.Constructor);
      }  
      ...
      internal static MethodInfo SelectMethod(MemberInfo[] match, ref Object[] args, String[] namedParameters){
        ...
        int n = match.Length;
        if (n == 0) return null;
        MemberInfo result = n == 1 ? match[0] : JSBinder.Select(match, n, ref args, namedParameters, MemberTypes.Method);
      ...
      internal static MethodInfo SelectMethod(MemberInfo[] match, IReflect[] argIRs){
        ...
        int n = match.Length;
        if (n == 0) return null;
        MemberInfo result = n == 1 ? match[0] : JSBinder.Select(match, n, argIRs, MemberTypes.Method);
        if (result != null && result.MemberType == MemberTypes.Property)
