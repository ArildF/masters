...
static int NameManglerLength; // count of characters in the hashed path, without the null
          ...
          void)
{
    ...
    if(init_count==0)
    {
        TRACE("Initialization count drops to zero, shutting down the PAL\n");
    ...
    {
        /* this is actually legal in some cases : on ExitProcess, PAL_Shutdown 
           makes the count reach zero, then LOADFreeModules will call DllMain, 
           from which PAL_Terminate may be called... is this acceptable? */
        WARN("PAL_Terminate called with no matching PAL_Initialize!\n");
    }
    SYNCLeaveCriticalSection(init_critsec, TRUE);
    LOGEXIT("PAL_Terminate returns\n");
...
Abstract:
  ...
  subdirectory of the user's home directory. ~/.rotor is created
  if it doesn't exist.
--*/
BOOL
PALAPI
...
returns TRUE on success, FALSE on failure; n case of failure, PAL_Initialize
must fail
...
BOOL INIT_InitPalConfigDir(void)
{
    ...
    /* Grab the full pathname to our librotor_pal.so
    ...
    if (lstat(librotor_fname, &librotor_stat) != -1)
    {
    ...
    /* Build the string used to mangle object names, which is unique
       based on the inode of the PAL, plus the name "rotor" */
    strcpy(hashpath, ROTOR_NAME);
    HashPath(hashpath+sizeof(ROTOR_NAME)-1, librotor_fname);
    ...
    /* Ignore the error code, as it may be ERROR_ALREADY_EXISTS (183) */
    CreateDirectoryW(PALUserDirName, NULL);
