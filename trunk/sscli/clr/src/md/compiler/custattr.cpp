...
{
    {SERIALIZATION_TYPE_BOOLEAN},{SERIALIZATION_TYPE_BOOLEAN}
};
    ...
    ULONG       cNamedArgs)             // Count of argument descriptors.
{
    ...
    LPCUTF8     pEnumName = NULL;       // Enum's name.
    ...
    {   
        ...
        for (ix=0; ix<cNamedArgs; ++ix)
        {
            ...
            if (ArgType == SERIALIZATION_TYPE_ENUM && (strncmp(pNamedArgs[ix].szEnumName, pEnumName, cbEnumName) != 0 ||
                pNamedArgs[ix].szEnumName[cbEnumName] != '\0'))
                continue;
            // Found a match.
            break;
        }
        ...
        case SERIALIZATION_TYPE_TYPE:
            pNamedArgs[ix].val.str.pStr = ca.GetString(&pNamedArgs[ix].val.str.cbStr);
            break;
    ...
    ULONG       cArgs)                  // Count of argument descriptors.
{
    ...
    {   
        ...
        case SERIALIZATION_TYPE_STRING:
        case SERIALIZATION_TYPE_TYPE:
            pArgs[ix].val.str.pStr = ca.GetString(&pArgs[ix].val.str.cbStr);
            break;
        ...
        if (strcmp(szNamespace, rKnownCaProps[i]->szNamespace) == 0)
        {
            ...
            if (rKnownCaProps[i]->bMatchBySig)
            {
                ...
                for (j=0; j<cParams; ++j)
                {
                    ...
                    pSig += cb;
                    cbSig -= cb;
                    if (rKnownCaProps[i]->pArgs[j].type != (CorSerializationType) elem)
                        break;
                }
    ...
    int         *bKeep)                 // [OUT] If true, keep the CA after processing.
{
    ...
    *bKeep = props->bKeepCa || m_bKeepKnownCa;
    ...
    case CA_DllImportAttribute: 
        ...
        IfFailGo(qDllName.ReSize(qArgs[0].val.str.cbStr+1));
        ...
        if (hr != S_OK)
        {
            ...
            IfFailGo(DefineModuleRef(wzDllName, &mrModule));
        }
        ...
        {
            MAKE_WIDEPTR_FROMUTF8N(wzEntryName, qNamedArgs[DI_EntryPoint].val.str.pStr, qNamedArgs[DI_EntryPoint].val.str.cbStr);
            wzEntry = wzEntryName;
        }
        else
        {
        ...
        IfFailGo(DefinePinvokeMap(tkObj, dwFlags, wzEntry, mrModule));
        goto ErrExit;
        }
        break;
