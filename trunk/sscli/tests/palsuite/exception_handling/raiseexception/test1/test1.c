...
int __cdecl main(int argc, char *argv[])
{
    ...
    PAL_EXCEPT_EX(TEST1, EXCEPTION_EXECUTE_HANDLER)
    {
        bExcept = TRUE;
    }
    PAL_ENDTRY;
    ...
    PAL_EXCEPT_EX(TEST2, EXCEPTION_EXECUTE_HANDLER)
    {
        if( bFinally == FALSE )
        {
            Fail("RaiseException: ERROR -> It appears the code in the "
                 "PAL_EXCEPT_EX executed before the code in PAL_FINALLY.\n");
        }
    ...
    {
        Trace("RaiseException: ERROR -> It appears the code in the "
              "PAL_EXCEPT_EX block was not executed.\n");
    }
