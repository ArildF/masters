    ...
    public sealed class FunctionExpression : AST{
      ...
      internal FunctionExpression(Context context, AST id, ParameterDeclaration[] formal_parameters, TypeExpression return_type, Block body, FunctionScope own_scope, FieldAttributes attributes)
        ...
        if (this.name.Length == 0)
          this.name = "anonymous "+(uniqueNumber++).ToString();
        else
      ...
      internal override AST PartiallyEvaluate(){
        ...
        if (scope != enclosingScope)
          //cut out scopes of outer functions. Early binding to locals in these scope will not work
      ...
      internal override void TranslateToIL(ILGenerator il, Type rtype){
        ...
        Convert.Emit(this, il, typeof(Closure), rtype);
        if (this.field != null){
          il.Emit(OpCodes.Dup);
      ...
      internal override void TranslateToILInitializer(ILGenerator il){
        ...
        this.func_local = il.DeclareLocal(typeof(FunctionObject));
        il.Emit(OpCodes.Ldtoken, compilerGlobals.classwriter);
