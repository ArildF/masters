...
//
//  Synopsis:   Determines if the passed in file is a C# file (by checking
//              the file extension).
//
//  Arguments:  [pszFile] -- Name of the file to test.
//
//----------------------------------------------------------------------------
    ...
    LPSTR pszFile)
{
    ...
    return (toupper(pszFile[len - 1]) == 'S' &&
            toupper(pszFile[len - 2]) == 'C' &&
            pszFile[len - 3] == '.');
}
            ...
            *pszfile = '\0';
            ...
            if (((_stricmp(pszsubdir, pszAssociateDir) != 0) &&
                ...
                *pszfile = PATH_SEPARATOR_CHAR;
                ...
                /*
                 * C# files actually build fine with a "prefix" directory, so
                 * don't complain in that case.
                 */
...
#ifdef PLATFORM_WIN32 // Only Win32 supports precompiled headers
