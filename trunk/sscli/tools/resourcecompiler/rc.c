...
#else
#define COMPILER_PREPROCESSOR_FLAGS             "/nologo /E /P /DRC_INVOKED=1 /DFEATURE_PAL=1"
...
LPSTR changeExtension(LPSTR filename, LPSTR old, LPSTR new);
...
int __cdecl main(int argc, LPSTR argv[])
{
  ...
  compilerOptions = malloc(sizeof(" " COMPILER_PREPROCESSOR_FLAGS " "));
  ...
  lineFile = malloc(strlen(filenameStart) + 1);
  if (lineFile == NULL) {
  ...
  strippedFilename = changeExtension(filenameStart, ".rc", ".i");

#if PLATFORM_UNIX
  {
    LPSTR tempString;
    tempString = (LPSTR) malloc(strlen(compilerOptions) +
                                strlen(" " TARGET_PREFIX " ") +
                                strlen(strippedFilename) +
                                    strlen(" ") + 1);
    if (tempString == NULL) {
                crashAndBurn("Out of memory - realloc'd compilerOptions",
                    UNKNOWN_LINE_NUMBER, "", "");
    }
    strcpy(tempString, compilerOptions);
    strcat(tempString, " " TARGET_PREFIX " ");
    strcat(tempString, strippedFilename);
    strcat(tempString, " ");
    compilerOptions = tempString;
  }
#endif
  ...
  while (fgets(textline, BUF_SIZE, strippedrc) != NULL){
    ...
    textlinetotok = malloc(strlen(textline) + 1);
    ...
    if (!strcmp(firstword, "STRINGTABLE")){
      ...
      while(1){
        ...
        if (stringtable == NULL){  //First time through loop, no memory yet.
          stringtable = malloc(sizeof(STRING_TABLE_ENTRY));
          if (stringtable == NULL){
        ...
        stringIDint = (UINT16) Expr_Eval();
        
        if (stringIDint == 0){
          crashAndBurn("STRINGTABLE in bad form or missing END", 
                       lineNumber, lineFile, textline);
        }
        ...
        if (stringValue == NULL){
          ...
          if (strchr(textline, '\"') != NULL){
            stringValue = strtok(strchr(textline, '\"'), "\"");
          }
          else{ 
        ...
        stringValueFormatted = FormatString(stringValue);
  ...
  for(i=0; i<stringTableCounter; i++){
    ...
    if (stringtable[i].StringID == lastValue){  //CHECK FOR DOUBLES
      ...
      errorMessage = malloc(sizeof("Duplicate StringID for Values: ") + 
    ...
    //PRINT OUT TO FILE
    OutFileWritten = TRUE;
    fprintf(outfile,"%u ",stringtable[i].StringID);
    fprintf(outfile,"\"");
    fprintf(outfile,stringtable[i].StringValue);
    fprintf(outfile,"\"");
    fprintf(outfile,"\n");
    lastValue = stringtable[i].StringID;
  }
  fprintf(outfile,"%c", EOF);
  ...
  while(argc){
    if (argc == 1){
      ...
      // Allow absolute paths
        ...
        inFilename = malloc(strlen(argv[0]) + 1);
        if (inFilename == NULL){
        ...
        inFileAppend = malloc(strlen(argv[0]) + sizeof(".rc"));
        ...
        if(inFile == NULL && inFile2 == NULL){
          ...
          errorMessage = malloc(sizeof("Cannot Open File: ") +
                                + strlen(inFilename));
          if (errorMessage == NULL)
      ...
      for(p = *argv + 1; *p != '\0'; p++){
        ...
        case 'd':
          ...
          *compilerOptions = realloc(*compilerOptions, 
...
void preProcessFile(LPSTR compilerOptions)
{
  ...
  commandLine = malloc(strlen(COMPILER_NAME) +
               strlen(compilerOptions) +
               strlen(inFilename) +
                       strlen("\"\"") + 1);
  if (commandLine == NULL){
...
LPSTR convertToUTF8(LPSTR inputString)
{
  ...
  numOfWideChars = MultiByteToWideChar(codepage, MB_PRECOMPOSED, 
                                       inputString, 
                                       strlen(inputString), 
                                       NULL, 0);
  
  wideString = malloc(numOfWideChars*sizeof(WCHAR));
  if (wideString == NULL){
    crashAndBurn("Out of memory - wideString", UNKNOWN_LINE_NUMBER, "", "");
  }

  i = MultiByteToWideChar(codepage,MB_PRECOMPOSED,inputString, 
  ...
  outputString = malloc(numOfOutChars + 1);
...
//
// Returns:
//   Pointer to a string in which % is replaced by %% and all character 
//  sequences interpretable as escape characters are replaced by the 
...
LPSTR FormatString(LPSTR inputString)
{
  ...
  // Max length is when all %s...becomes twice as long

  formattedString = malloc(2*strlen(inputString) + 1);
...
LPSTR changeExtension(LPSTR filename, LPSTR old, LPSTR new)
{
  ...
  returnValue = malloc(strlen(filename) + strlen(new));
  if (returnValue == NULL)
    crashAndBurn("Out of memory - returnValue", UNKNOWN_LINE_NUMBER, "", "");

  returnValue = strcpy(returnValue, filename);

  tempString = strstr(returnValue, old);
  if (tempString != NULL)
    tempString[0] = '\0';
  returnValue = strcat(returnValue, new);
...
void updateLineNumber(LPSTR line, int * lineNumber, LPSTR * lineFile){
  ...
  linetotok = malloc(strlen(line) + 1);
  if (linetotok == NULL)
  ...
  *lineNumber = strtoul(lineNumberString, NULL, 0) - 1;
  ...
  fileTokenCopy = malloc (strlen(fileToken) + 1);
...
#else
    return (strstr(line, LINEMARKER_PREFIX) == line);
#endif
}
