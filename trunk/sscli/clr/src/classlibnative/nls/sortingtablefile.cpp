...
const WCHAR SortingTable::m_wszSortKeyMappingName[]   = L"_nlsplus_sortkey_1_0_3310_nlp";

const CHAR  SortingTable::m_szSortTableFileName[]     = "sorttbls.nlp";
const WCHAR SortingTable::m_wszSortTableMappingName[] = L"_nlsplus_sorttbls_1_0_3310_nlp";
...
// supported in NLS+.
const int  SortingTable::m_nLangIDCount   = 136;
const int  SortingTable::m_nLangArraySize = m_nLangIDCount + 1;
...
const BYTE SortingTable::m_SortingTableOffset[] =
{
...
void SortingTable::GetSortInformation()
{
    ...
    m_pSortTable = pBaseAddr = (LPWORD)m_pNativeGlobalizationAssembly->MapDataFile(m_wszSortTableMappingName, m_szSortTableFileName, &m_hSortTable);
...
PSORTKEY SortingTable::GetDefaultSortKeyTable(HANDLE *pMapHandle) {
    ...
    if (m_pDefaultSortKeyTable == NULL)
    {
        ...
        m_pDefaultSortKeyTable = (PSORTKEY)((LPWORD)m_pNativeGlobalizationAssembly->MapDataFile(
            m_wszSortKeyMappingName, m_szSortKeyFileName, pMapHandle) + SORTKEY_HEADER);
    }
    ...
    *ppIdeograph = NULL;
    ...
    for (ctr = m_NumIdeographLcid; ctr > 0; ctr--, pIdeoLcid++)
    {
        if (pIdeoLcid->Locale == (DWORD)nLcid)
        {
            ...
            //
            HANDLE hFileMapping;
            *ppIdeograph = m_pNativeGlobalizationAssembly->MapDataFile((WCHAR *) pIdeoLcid->pFileName, (WCHAR *) pIdeoLcid->pFileName, &hFileMapping);
