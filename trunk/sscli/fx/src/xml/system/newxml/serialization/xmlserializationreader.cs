...
namespace System.Xml.Serialization {
    ...
    public abstract class XmlSerializationReader {
        ...
        Type GetPrimitiveType(XmlQualifiedName typeName) {
            InitPrimitiveIDs();
        ...
        protected bool GetNullAttr() {
            string isNull = r.GetAttribute(nullID, instanceNsID);
            if(isNull == null)
                isNull = r.GetAttribute(nilID, instanceNsID);
            if (isNull == null) {
        ...
        protected object ReadReferencingElement(string name, string ns, bool elementCanBeType, out string fixupReference) {
            ...
            if (callbacks == null) {
                ...
                types = new Hashtable();
                types.Add(new XmlQualifiedName(urTypeID, r.NameTable.Add(XmlSchema.Namespace)), typeof(object));
                InitCallbacks();
            }
    ...
    internal class XmlSerializationReaderCodeGen {
        ...
        class CreateCollectionInfo {
            ...
            public CreateCollectionInfo(string name, TypeDesc td) {
                this.name = name;
                this.td = td;
            }
            public string Name {
        ...
        class Member {
            ...
            public Member(string source, string arrayName, int i, MemberMapping mapping) 
                : this(source, null, arrayName, i, mapping, false, null) {
            }
            public Member(string source, string arrayName, int i, MemberMapping mapping, string choiceSource)
                : this(source, null, arrayName, i, mapping, false, choiceSource) {
            }
            public Member(string source, string arraySource, string arrayName, int i, MemberMapping mapping) 
                : this (source, arraySource, arrayName, i, mapping, false, null) { 
            }
            public Member(string source, string arraySource, string arrayName, int i, MemberMapping mapping, string choiceSource) 
                : this (source, arraySource, arrayName, i, mapping, false, choiceSource) { 
            }
            public Member(string source, string arrayName, int i, MemberMapping mapping, bool multiRef)
                : this(source, null, arrayName, i, mapping, multiRef, null) {
            }
            public Member(string source, string arraySource, string arrayName, int i, MemberMapping mapping, bool multiRef, string choiceSource) {
                this.source = source;
            ...
            public string ParamsReadSource {
            ...
            public string ChoiceArrayName {
                get { return choiceArrayName; }
            }
            public string ChoiceArraySource {
        ...
        public string GenerateElement(XmlMapping xmlMapping) {
        ...
        void GenerateInitCallbacksMethod() {
            ...
            foreach (TypeScope scope in scopes) {
                foreach (TypeMapping mapping in scope.TypeMappings) {
                    ...
                    if (mapping.IsSoap && (mapping is StructMapping || mapping is EnumMapping)) {
        ...
        string GenerateLiteralMembersElement(XmlMembersMapping xmlMembersMapping) {
            ...
            ArrayList attrOrArrayMembersList = new ArrayList();
            ...
            for (int i = 0; i < mappings.Length; i++) {
                ...
                if (mapping.Attribute != null || mapping.Text != null || mapping.Xmlns != null)
                    attrOrArrayMembersList.Add(member);
                for (int j = 0; j < mapping.Elements.Length; j++) {
                    if (mapping.Elements[j].Any) {
                        anyElement = anyMember;
                        if (mapping.Attribute == null && mapping.Text == null)
                            attrOrArrayMembersList.Add(anyMember);
            ...
            Member[] attrOrArrayMembers = (Member[]) attrOrArrayMembersList.ToArray(typeof(Member));

            if (attrOrArrayMembers.Length > 0) {
                WriteMemberBegin(attrOrArrayMembers);

                writer.WriteLine("while (Reader.MoveToNextAttribute()) {");
                writer.Indent++;
                WriteAttributes(attrOrArrayMembers, anyAttribute, "UnknownNode((object)p);");
                writer.Indent--;
                writer.WriteLine("}");
                if (anyAttribute != null)
                    WriteMemberEnd(attrOrArrayMembers);
            ...
            WriteWhileNotEndTagWithParamsRead(members);
            writer.Indent++;
        ...
        string GenerateEncodedMembersElement(XmlMembersMapping xmlMembersMapping) {
            ...
            if (members.Length > 0 && members[0].Mapping.IsReturnValue) writer.WriteLine("bool isReturnValue = true;");
        ...
        void WriteLiteralStructMethod(StructMapping structMapping) {
            ...
            if (structMapping.TypeDesc.IsAbstract) {
                ...
                WriteWhileNotEndTagWithParamsRead(allMembers);
                writer.Indent++;
        ...
        void WriteAttributes(Member[] members, Member anyAttribute, string attrElse) {
            ...
            for (int i = 0; i < members.Length; i++) {
                ...
                if (member.Mapping.CheckSpecified) { 
                    writer.Write("o.");
                    CodeIdentifier.CheckValidIdentifier(member.Mapping.Name);
                    writer.Write(member.Mapping.Name);
        ...
        void WriteMemberElementsElse(Member anyElement, string elementElseString) {
            if (anyElement != null) {
                ...
                for (int i = 0; i < elements.Length; i++) {
                    ...
                    if (element.Any) {
                        WriteElement(anyElement.ArraySource, anyElement.ArrayName, null, element, anyElement.Mapping.ChoiceIdentifier, null, false, false, -1, i);
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < members.Length; i++) {
                ...
                for (int j = 0; j < elements.Length; j++) {
                    ...
                    if (checkType) {
                        ...
                        WriteElement(member.ArraySource, member.ArrayName, member.ChoiceArraySource, e, choice, member.Mapping.CheckSpecified ? member.Mapping.Name : null, member.IsList && member.Mapping.TypeDesc.IsNullable, member.Mapping.ReadOnly, member.FixupIndex, j);
                    }
                    if (e.IsSoap && member.Mapping.IsReturnValue)
        ...
        void WriteWhileNotEndTagWithParamsRead(Member[] members) {
            writer.Write("bool[] paramsRead = new bool[");
            writer.Write(members.Length.ToString());
            writer.WriteLine("];");
            writer.WriteLine("Reader.MoveToContent();");
            writer.Write("while (Reader.NodeType != ");
            writer.Write(typeof(XmlNodeType).FullName);
        ...
        void WriteCatchCastException(TypeDesc typeDesc, string source) {
            ...
            writer.WriteLine("}");
            writer.Write("catch (");
            writer.Write(typeof(InvalidCastException).FullName);
            writer.WriteLine(") {");
            writer.Indent++;
