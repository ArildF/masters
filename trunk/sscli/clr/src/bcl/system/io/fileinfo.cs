...
namespace System.IO {    
	...
	public FileStream Create() {
        ...
        public void MoveTo(String destFileName) {
    		...
    		destFileName = Path.GetFullPathInternal(destFileName);
    		new FileIOPermission(FileIOPermissionAccess.Write, new String[] { destFileName }, false, false).Demand();
    		
            if (!Win32Native.MoveFile(FullPath, destFileName))
    			__Error.WinIOError();
            FullPath = destFileName;
            _name = Path.GetFileName(destFileName);
        }
