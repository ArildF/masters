...
namespace System.IO {
    ...
    public sealed class Path
    {
        ...
        internal static String GetFullPathInternal(String path) {
            ...
            String newPath;
            int hresult = nGetFullPathHelper( path, InternalInvalidPathChars, String.WhitespaceChars, DirectorySeparatorChar, AltDirectorySeparatorChar, true, out newPath );
            if (hresult != 0)
                __Error.WinIOError(hresult, path);

            // Fixup path like " d:\" to be user friendly since after canonacalization we can fix this up.
            // We will not create a new String unless we have to so this is not a problem.
            newPath = newPath.TrimStart();
            return newPath;
        }

        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern int nGetFullPathHelper( String path, char[] invalidPathChars, char[] whitespaceChars, char directorySeparator, char altDirectorySeparator, bool fullCheck, out String newPath );
        ...
        {
            String newPath;
            int hresult = nGetFullPathHelper( path, InternalInvalidPathChars, String.WhitespaceChars, DirectorySeparatorChar, AltDirectorySeparatorChar, false, out newPath );
            if (hresult != 0)
            {
                __Error.WinIOError(hresult, path);
            }
