...
public class Co8617set_Capacity
{
    ...
    public bool runTest()
    {
        ...
        try
        {
            strLoc = "Loc_384sdg";
            iCountTestcases++;
            iCurrentDefaultCapacity = 16;
            list = new SortedList();
            if(list.Capacity!=iCurrentDefaultCapacity)
            {
                iCountErrors++;
                Console.WriteLine( "Err_9347sdg! wrong value returned. Expected - <{0}>, Returned - <{1}>", 0, list.Capacity);
            }
            list.Capacity = 3;
            if(list.Capacity!=3)
            {
                iCountErrors++;
                Console.WriteLine( "Err_987345sg! wrong value returned. Expected - <{0}>, Returned - <{1}>", 0, list.Capacity);
            }
            iCountTestcases++;
            ...
            {
                iCountErrors++;
                Console.WriteLine( "Err_2947sdg! wrong value returned. Expected - <{0}>, Returned - <{1}>", 0, list.Capacity);
            }
            capacity = 5000;
            list.Capacity = capacity;
            if(list.Capacity!=capacity)
            {
                iCountErrors++;
                Console.WriteLine( "Err_34058wgsd! wrong value returned. Expected - <{0}>, Returned - <{1}>", 0, list.Capacity);
            }
            iCountTestcases++;
            list = new SortedList();
            capacity = 0;
