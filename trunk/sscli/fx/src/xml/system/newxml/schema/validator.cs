    ...
    internal sealed class Validator {
        ...
        private void PushElementDecl(string ns, string prefix, int depth) {
            ...
            if (nextElement != null) {
                ...
                //                                  set the context here, so the stack can have them
                if (nextElement.Constraints != null) {
                    ...
                    //                                  make connections between new keyref tables with key/unique tables in stack
                ...
                if (this.startIDConstraint != -1) {
                    for (int ci = this.validationStack.Length - 1; ci >= this.startIDConstraint; ci --) {
                        ...
                        foreach (ConstraintStruct conuct in ((ValidationState)(this.validationStack[ci])).Constr) {
                            // EndChildren
                            conuct.axisSelector.EndElement(reader.LocalName, reader.NamespaceURI);
                            ...
                            foreach (LocatedActiveAxis laxis in conuct.axisFields) {
                                ...
                                if (laxis.isMatched) {
                                    ...
                                    if (((KeySequence) (conuct.constraintTable[laxis.Row]))[laxis.Column] != null) {
                                        ...
                                        ((KeySequence) (conuct.constraintTable[laxis.Row]))[laxis.Column] =
                                            new TypedObject (reader.TypedValueObject, stringValue, nextElement.Datatype);
                                    }
                                }
                                // EndChildren
                    ...
                    if ( vcs != null) {
                        // start to checking confliction of tables.... key, unique, keyref...
                        // using hashtable to do this...

                        // firstly create all key/unique/keyref qualified hashtables
                        foreach (ConstraintStruct conuct in vcs) {
                            ...
                            case CompiledIdentityConstraint.ConstraintRole.Key:
                                foreach (KeySequence ks in conuct.constraintTable) {
                                    if (! ks.IsQualified()) {
                                    ...
                                    else if (conuct.qualifiedTable.ContainsValue (ks)) {
...
//                                            ks2.PosLine.ToString(), ks2.PosCol.ToString()},
                            ...
                            case CompiledIdentityConstraint.ConstraintRole.Unique:
                                foreach (KeySequence ks in conuct.constraintTable) {
                                    ...
                                    if (conuct.qualifiedTable.ContainsValue (ks)) {
...
//                                            ks2.PosLine.ToString(), ks2.PosCol.ToString()},
                        ...
                        // secondly validating all referencing tables...
                        foreach (ConstraintStruct conuct in vcs) {
                            ...
                            foreach (Hashtable keyrefTable in conuct.keyrefTables) {
                                foreach (KeySequence ks in keyrefTable.Values) {
                                    if (! conuct.qualifiedTable.ContainsValue (ks)) {
                ...
                if (!en.IsProcessed) {
                    ...
                    if (en.IsExternal) {
                        ...
                        if (!fInAttribute) {
                            ...
                            Uri uri = this.xmlResolver.ResolveUri(((null == baseuri || String.Empty == baseuri) ? null : new Uri(en.BaseURI)), en.Url);
                            Stream stm = (Stream)this.xmlResolver.GetEntity(uri, null, null);
                            resolvedUrl = nameTable.Add(uri.ToString());
                            en.IsProcessed = true;
                        ...
                        resolvedUrl = (en.DeclaredURI != null) ? nameTable.Add(en.DeclaredURI) : string.Empty;
                        if (en.Text.Length > 0) {
                            en.IsProcessed = true;
                        ...
                        foreach (LocatedActiveAxis laxis in conuct.axisFields) {
                            ...
                            if (laxis.MoveToAttribute(name, ns)) {
                                ...
                                if (((KeySequence) (conuct.constraintTable[laxis.Row]))[laxis.Column] != null) {
                                    ...
                                    ((KeySequence) (conuct.constraintTable[laxis.Row]))[laxis.Column] =
                                        new TypedObject (obj, sobj, attdef.Datatype);
                                }
                            }
                        }
                    }
