...
*/
		...
		{
			Util.Log("WsdlParser.ParseReaderStreamLocation");           
...
*/
		...
		private void ParseImport()
		{
			...
			String location = LookupAttribute(s_locationString, null, false);
		...
		{
			Util.Log("WsdlParser.ParseInput");                                  
			_parsingInput = input;
			try
			{
				ReadNextXmlElement();
				...
				if (MatchingNamespace(s_wsdlNamespaceString) &&
					MatchingStrings(elementName, s_definitionsString))
				{
					ParseWsdl();
				}
				else
		...
		private void PruneNamespaces()
		{
			...
			//Keep only namespaces which are used.

			ArrayList newNamespaces = new ArrayList();
			IDictionaryEnumerator denum = namespaces.GetEnumerator();
			while(denum.MoveNext())
			{
				String item = (String)denum.Key;
				Util.Log("WsdlParser.PruneNamespaces Found "+item);                       
				URTNamespace ns = LookupNamespace(item);
				if (ns != null)
				{
					newNamespaces.Add(ns);
					Util.Log("WsdlParser.PruneNamespaces Kept "+item);                       
				}
				else
				{
					Util.Log("WsdlParser.PruneNamespaces deleted "+item);                       
				}

			}
			*/
			_URTOldNamespaces = _URTNamespaces;	//only used the namespaces which are references
			_URTNamespaces = new ArrayList();
		...
		private void ResolveWsdlClass(WsdlBinding binding, WsdlBindingSuds suds, bool bFirstSuds)
		{
			...
			URTNamespace parsingNamespace = AddNewNamespace(binding.typeNs);
		...
		{
			Util.Log("WsdlParser.lookupNamespace name "+name);            
			for (int i=0;i<_URTNamespaces.Count;i++)
			{
				...
				//if (urtNS.Name == name)
				{
					// Util.Log("WsdlParser.lookupNamespace search ns found "+urtNS.Name+" input "+name);            
					return(urtNS);
				}
			}
		...
		private URTNamespace LookupOldNamespace(String name)
		{
			if (_URTOldNamespaces != null)
			{
				//Util.Log("WsdlParser.lookupNamespace name "+name);
				for (int i=0;i<_URTOldNamespaces.Count;i++)
				{
					URTNamespace urtNS = (URTNamespace) _URTOldNamespaces[i];
					if (WsdlParser.MatchingStrings(urtNS.Name, name))
						return(urtNS);
				}
			}
		...
		// The first pass finds all the types and namespaces
		// The second pass starts from the Soap/Rpc/Encoded binding section
		// and only uses the Namespaces which are referenced from that 
		// binding. This will eliminate all the types from bindings
		// we don't use.
		internal URTNamespace AddNewNamespace(String ns)
		{
			...
			if (ns != null)
			{
				xns = LookupOldNamespace(ns);

				if (xns == null)
				{
					xns = LookupNamespace(ns);
					if (xns == null)
					{
						new URTNamespace(ns, this);
				...
				{
					Util.Log("WsdlParser.AddNewNamespace old namespace transfered "+ns);            
					RemoveOldNamespace(xns);
					AddNamespace(xns);
				}
		...
		{
			Util.Log("WsdlParser.AddNamespace "+xns.Namespace);
			_URTNamespaces.Add(xns);
		}

		private void RemoveOldNamespace(URTNamespace xns)
		{
			_URTOldNamespaces.Remove(xns);
		}
		...
		private ArrayList _URTOldNamespaces;
		...
		internal class URTComplexType : BaseType
		{
			...
			: base(name, urlNS, ns, encodedNS)
			{
				Util.Log("URTComplexType.URTComplexType name "+name+" urlNS "+urlNS+" ns "+ns+" encodedNS "+encodedNS+" bSUDStype "+bSUDSType+" bAnonymous "+bAnonymous);
				_baseTypeName = null;
				_baseTypeXmlNS = null;
				_baseType = null;
				_connectURLs = null;
			...
			internal void PrintCSC(TextWriter textWriter, String indentation, String curNS, StringBuilder sb)
			{
				...
				bool binherit = false;
				bool bBaseClientProxy = (_baseTypeName == null || _baseTypeName == "MarshalByRefObject" || _baseTypeName == "__ComObject" || _baseTypeName == "RemotingClientProxy");
				if (fClientProxy && bBaseClientProxy)
				{
					// Case should not occur since _baseTypeName is set to RemotingClientProxy
				...
				if (fClientProxy)
					PrintClientProxy(textWriter, indentation, curNS, sb, bBaseClientProxy);
			...
			private void PrintClientProxy(TextWriter textWriter, String indentation, String curNS, StringBuilder sb, bool bBaseClientProxy)
			{
			...
			{
				Util.Log("URTComplexType.ResolveArray Entry "+_arrayType+" ns "+_arrayNS);
		...
		internal class URTNamespace
		{
			...
			internal URTNamespace(String name, WsdlParser parser)
			{
				...
				Util.Log("URTNamespace.URTNamespace Exit name "+name+" _namespace "+_namespace+" _assemName "+_assemName+" _encodedNS "+_encodedNS);
			}
			...
			internal void AddComplexType(URTComplexType type){
				Util.Log("URTNamespace.AddComplexType "+type.Name+" ns "+_name);
			...
			internal URTComplexType LookupComplexType(String typeName){
				...
				Util.Log("URTNamespace.LookupComplexType "+typeName+" ns "+_name+" return "+((ctype != null) ? ctype.Name:"null"));               
				return(ctype);
			}
			...
			internal URTComplexType LookupComplexTypeEqual(String typeName){
				...
				Util.Log("URTNamespace.LookupComplexTypeEqual "+typeName+" ns "+_name+" return "+((ctype != null) ? ctype.Name:"null"));               
				return(ctype);
			}
			...
			internal void PrintCSC(TextWriter textWriter){
