...
namespace System.Security {
    ...
    internal class CodeAccessSecurityEngine
    {
        ...
        internal virtual void Assert(CodeAccessPermission cap, ref StackCrawlMark stackMark)
        {
            ...
            BCLDebug.Assert(secObj != null || !SecurityManager.SecurityEnabled,"Failure in CodeAccessSecurityEngine.Assert() - secObj != null");
            if (secObj == null)
            {
        ...
        internal virtual void Deny(CodeAccessPermission cap, ref StackCrawlMark stackMark)
        {
            ...
            BCLDebug.Assert(secObj != null || !SecurityManager.SecurityEnabled,"Failure in CodeAccessSecurityEngine.Deny() - secObj != null");
            if (secObj == null)
            {
                if (SecurityManager.SecurityEnabled)
                    // Security: REQ_SQ flag is missing. Bad compiler ?
