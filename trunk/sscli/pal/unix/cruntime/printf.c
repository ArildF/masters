...
static size_t  Internal_Convertfwrite(const void *buffer, size_t size, size_t count , FILE *stream,BOOL convert)
{
   int ret;
    ...
    if(convert)
    {
        int nsize;
        LPSTR newBuff = 0;
        nsize = WideCharToMultiByte(CP_ACP, 0,buffer, count, 0, 0, 0, 0);
        if (!nsize)
        {
        ...
        nsize = WideCharToMultiByte(CP_ACP, 0, buffer, count,
                                newBuff, nsize, 0, 0);
        if (!nsize)
        {
            ASSERT("WideCharToMultiByte failed.  Error is %d\n",
      ...
      va_list ap)
{
    ...
    Length = PAL_vfprintf( PAL_get_stdout(), format, ap);
     ...
     ...)
{
    ...
    ENTRY("fwprintf (stream=%p, format=%p (%S))\n",
          stream, format, format);
                                                ...
                                                &Prefix, &Type))
        {
                 ...
                 (Type == PFF_TYPE_WSTRING && (Flags & PFF_ZERO) != 0))
            {
                ...
                else
                {
                    ...
                    if ( Length != 0 )
                    {
                        ...
                        {
                            ERROR( "malloc failed.\n" );
                            LOGEXIT("fwprintf returns int -1\n");
                            return -1;
                        }
                    }
                    else
            ...
            else
            {
                ...
                if (Type == PFF_TYPE_P && Prefix == PFF_PREFIX_SHORT)
                {
                    ...
                    {
                        if (NULL == (TempSprintfStrPtr = malloc(++TempInt)))
                        {
                            ERROR("malloc failed\n");
                            LOGEXIT("fwprintf returns int -1\n");
                            SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    ...
    LOGEXIT("fwprintf returns int %d\n", written);
    return (written);
}
