...
namespace System.Security.Policy {
    ...
    sealed public class NetCodeGroup : CodeGroup
    {
        ...
        private SecurityElement CreateSocketPermission( String host, String scheme )
        {
            SecurityElement root = new SecurityElement( "IPermission" );

            root.AddAttribute( "class", "System.Net.SocketPermission, System, Version=" + Assembly.GetExecutingAssembly().GetVersion().ToString() + ", Culture=neutral, PublicKeyToken=" + AssemblyRef.EcmaPublicKeyToken);
        ...
        private SecurityElement CreateWebPermission( String host, String scheme )
        {
            ...
            root.AddAttribute( "class", "System.Net.WebPermission, System, Version=" + Assembly.GetExecutingAssembly().GetVersion().ToString() + ", Culture=neutral, PublicKeyToken=" + AssemblyRef.EcmaPublicKeyToken);
            root.AddAttribute( "version", "1" );
            ...
            if (scheme != null && String.Compare( scheme, "http", true, CultureInfo.InvariantCulture) == 0)
                uriStr = "(https|http)://" + EscapeSiteString( host ) + "/.*";
            else if (scheme != null)
                uriStr = scheme + "://" + EscapeSiteString( host ) + "/.*";
            else
                uriStr = ".*://" + EscapeSiteString( host ) + "/.*";
