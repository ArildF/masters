...
void MachState::Init(void **prVal[NUM_CALLEESAVED_REGISTERS],
                     void *aSp,
                     void **aPRetAddr)
{
#ifdef _DEBUG
...
void LazyMachState::getState(int funCallDepth, TestFtn testFtn)
{
    ...
    memcpy(&Gprs[13], _Regs, sizeof(_Regs));
    for (i=0; i<32; ++i) {
        pGprs[i] = &Gprs[i];
    }
    Cr = _cr;

    Gprs[1] = (unsigned __int32)captureSp;  // r1 is the stack pointer
    pGprs[1] = NULL;
    Lr = 0;
    pLr = &Lr;
    ip = capturePC;
...
#define MARK_UNINITIALIZED(r)
        ...
        case 31:
            ...
            case 4:     // tw (aka breakpoint)
                // If you get at this error, it is because you
        ...
        case 32:        // lwz rD,d(rA)
            ...
            if (rA == 1) {     // lwz rD,d(sp)
                ...
                if (dp == (BYTE*)captureSp) {
                    pGprs[rD] = NULL;
                    Gprs[rD] = (unsigned __int32)captureSp2;
                }
                else {
...
badOpcode:
    ...
    // state when the the helper returns to its caller.  _pRetVal must be
    // updated last, as it is the signal that the entire instance is valid.

    memcpy(_Regs, &Gprs[13], sizeof(_Regs));
    for (i=0; i<NUM_CALLEESAVED_REGISTERS; ++i) {
        if (pGprs[13+i] == &Gprs[13+i]) {
            _pRegs[i] = &_Regs[i];
        } else {
            _pRegs[i] = (void**)pGprs[13+i];
        }
    }
    _cr = Cr;
    _sp = (void*)Gprs[1];
    *(volatile void ***)&_pRetAddr = (volatile void**)pRetAddr;
}
