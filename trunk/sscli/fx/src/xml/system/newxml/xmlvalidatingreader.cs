...
namespace System.Xml
{
    ...
    public class XmlValidatingReader : XmlReader, IXmlLineInfo {
        ...
        :this(new XmlTextReader(xmlFragment, fragType, context))
        {
            ...
            _BaseURI = (null != context ? _CoreReader.NameTable.Add(context.BaseURI) : String.Empty);
                            ...
                            while (_CoreReader.MoveToNextAttribute()) {
                                if (_CoreReader.ValueContainEntity == ValueContainEntity.NotResolved) {
                                    if (_CoreReader.IsXmlNsAttribute)  {
                                        ExpandLiteralValue();
                                        NamespaceManager.AddNamespace(_CoreReader.LocalName, _CoreReader.Value);
                                    }
                                    else {
                                        ExpandLiteralValue();
                                    }
        ...
        /// <include file='doc\XmlValidatingReader.uex' path='docs/doc[@for="XmlValidatingReader.ReadInnerXml"]/*' />
        /// <devdoc>
        ...
        public override String ReadInnerXml() {
            ...
            switch (this.NodeType) {
                case XmlNodeType.Element:
                case XmlNodeType.Attribute:
                    value = _CoreReader.ReadInnerXml();
                    UpdateNodeProperties(_CoreReader);
                    break;
        ...
        /// <include file='doc\XmlValidatingReader.uex' path='docs/doc[@for="XmlValidatingReader.ReadOuterXml"]/*' />
        /// <devdoc>
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        public override String ReadOuterXml() {
            ...
            switch (this.NodeType) {
                case XmlNodeType.Element:
                case XmlNodeType.Attribute:
                    value = _CoreReader.ReadOuterXml();
                    UpdateNodeProperties(_CoreReader);
                    break;
                case XmlNodeType.None:
                    value = String.Empty;
        ...
        public override string ReadString() {
            ...
            if (_CoreReader.IsReadingAttributeValue) {
                MoveToElement();
            }

            if (((NodeType == XmlNodeType.Element || NodeType == XmlNodeType.Attribute) && !_IsEmptyElement) ||
                (NodeType == XmlNodeType.EntityReference && false == _ResolveEntityInternally)) {
                if (! Read())
                    throw new InvalidOperationException(Res.GetString(Res.Xml_InvalidOperation));
            }
        ...
        private void PushXmlReader(Object entity, XmlTextReader reader) {
        ...
        private void ExpandLiteralValue() {
            ...
            PushXmlReader(null, r);
            r.Read();
            CollectTextToken(EntityHandling.ExpandEntities, depth, true);
            PopXmlReader();
            _CoreReader.AttributeValue = _StringBuilder.ToString();
            ...
            do {
                switch (_CoreReader.NodeType) {
                    case XmlNodeType.Whitespace:
                        ...
                        _StringBuilder.Append(_CoreReader.Value);
                        break;
                    case XmlNodeType.Text:
                        if (_StringBuilder.Length == 0) {
                            _LineNumber = _CoreReader.LineNumber;
                    ...
                    case XmlNodeType.EntityReference:
                        if (ehMode == EntityHandling.ExpandEntities) {
                            ...
                            if (scanner != null) {
                                PushXmlReader(o, new XmlTextReader(scanner, this.NameTable, _CoreReader.NamespaceManager, (isAttText ? XmlNodeType.Attribute : XmlNodeType.Element), depth, _CoreReader.Encoding, resolved, external, _CoreReader.StandAlone));
                            }
                            else {
                                returnFirstNodeInEntityRef = false;
