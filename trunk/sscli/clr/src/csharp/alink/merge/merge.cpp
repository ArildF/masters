...
HANDLE OpenFileEx( LPCWSTR filename, DWORD *fileLen, LPCWSTR relPath, bool bWriteAlso)
{
    ...
    // Try a relative path
    if (local == INVALID_HANDLE_VALUE && relPath != NULL && *relPath != 0) {
        WCHAR drive[_MAX_DRIVE], dir[_MAX_DIR];

        _wsplitpath( filename, drive, dir, NULL, NULL);
        if ( drive[0] == 0 && dir[0] != L'\\') {
            WCHAR newFilename[MAX_PATH];

            // strip off any filename in the relPath
            _wsplitpath( relPath, drive, dir, NULL, NULL);
            // Prepend the relPath info on the original filename
            _wmakepath( newFilename, drive, dir, filename, NULL);

            // Now try again
            if (bWriteAlso)
                local = W_CreateFile( newFilename, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            else
                local = W_CreateFile( newFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
        }
    }
...
bool CTokenMap::ReMapToken(mdToken *pToken, bool fSwap)
{
    ...
    if (fSwap)
    {
        *pToken = VAL32(found->tkNewToken);
    }
    else
    {
        *pToken = found->tkNewToken;
    }
    return true;
}
...
HRESULT CTokenMap::Map(mdToken oldToken, mdToken newToken)
{
    ...
    if (!m_Tokens.Add(newMap, _TokenMap::CompareOld)) {
        _TokenMap *found;
...
PIMAGE_SECTION_HEADER ALPEFile::ImageNextSection(PIMAGE_SECTION_HEADER section)
{
...
PIMAGE_SECTION_HEADER ALPEFile::ImageRvaToSection(ULONG Rva)
{
    ULONG i, dwStop;
    PIMAGE_SECTION_HEADER NtSection;
...
PVOID ALPEFile::ImageRvaToVa(ULONG Rva)
{
...
HRESULT ALPEFile::OpenFileAs(IMetaDataDispenserEx *pDisp, LPCWSTR pszFileName, bool bAllowNonIl, bool bWriteable)
{
    HRESULT             hr = E_FAIL;
    DWORD               dwRead = 0;
...
HRESULT ALPEFile::EmitMembers(ICeeFileGen *pOutFile, HCEEFILE hCeeFile, HCEESECTION hIlSection, HCEESECTION hResSection, DWORD * pdwOffset, CTokenMap *pMap, IMetaDataEmit *pEmit)
{
#define COUNT   32
    ...
    ULONG                       cTypes = 0, cMembers = 0, cRes = 0;
...
ENUM_MEMBERS:
    ...
    do {
        ...
        for (ULONG iRes = 0; iRes < cRes; iRes++) {
            DWORD cchLen;
...
OpCode *ALPEFile::FindOpCode(BYTE *code)
{
...
HRESULT ALPEFile::FindTokens(void *pMethod, CTokenMap *pMap)
{
    ASSERT(pMethod);
    ...
    if (SUCCEEDED(hr) && meth.EHCount() > 0) {
        ...
        COR_ILMETHOD_SECT_EH_CLAUSE_FAT *fat;
        COR_ILMETHOD_SECT_EH_CLAUSE_SMALL *tiny;
        ...
        do {
            ...
            if (current->IsFat()) {
                ...
                for (index = 0; index < count; index++) {
                    if (fat[index].GetFlags() == COR_ILEXCEPTION_CLAUSE_NONE)
                        pMap->ReMapToken((mdToken*)&fat[index].ClassToken, true);
                }
            } else {
                tiny = current->Small.Clauses;
                for (index = 0; index < count; index++) {
                    if (tiny[index].GetFlags() == COR_ILEXCEPTION_CLAUSE_NONE)
...
HRESULT ALPEFile::WriteNewMetaData(IMetaDataEmit *pEmit, DWORD newSize)
{
    DWORD oldSize;
    PVOID md = NULL;
    HRESULT hr;
...
ALPEFile::ALPEFile() : pICH(NULL), pbMapAddress(NULL), dwLength(0), pImport(NULL), bIs64(false), pNT32(NULL)
{
}
