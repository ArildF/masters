...
The InterlockedExchangePointer function atomically exchanges a pair of
values. The function prevents more than one thread from using the same
variable simultaneously.
...
Target 
       [in/out] Pointer to the pointer to the value to exchange. The function sets
       this variable to value, and returns its prior value.
Value 
...
ASMFUNC InterlockedExchangePointer
    lwarx a2,0,a0
    stwcx. a1,0,a0
    bne _InterlockedExchangePointer
    sync
    mr a0,a2
    blr
...
values, based on the outcome of the comparison. More than one thread
...
Destination     [in,out] Pointer to the pointer to the destination value. The sign is ignored. 
Exchange        [in]     Specifies the exchange value. The sign is ignored. 
Comperand       [in]     Specifies the value to compare to Destination. The sign is ignored. 
...
--*/
ASMFUNC InterlockedCompareExchangePointer
    lwarx a3, 0, a0
    cmpw a3,a2
    bne ContP
    stwcx. a1,0,a0
    bne _InterlockedCompareExchangePointer
ContP:
    sync
    mr a0,a3
    blr
