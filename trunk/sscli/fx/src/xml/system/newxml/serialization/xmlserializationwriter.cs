...
namespace System.Xml.Serialization {
    ...
    public abstract class XmlSerializationWriter {
        ...
        internal void Init(XmlWriter w, ArrayList namespaces) {
            this.w = w;
        ...
        protected void WriteNullTagEncoded(string name, string ns) {
            ...
            w.WriteAttributeString("null", XmlSchema.InstanceNamespace, "1");
            w.WriteEndElement();
        }
        ...
        private void WriteElement(XmlNode node, string name, string ns, bool any) {
            ...
            if (any) {
                if (node is XmlElement && name != null && name.Length > 0) {
                    ...
                    if (node.Name != name || node.NamespaceURI != ns)
                        throw new InvalidOperationException(Res.GetString(Res.XmlElementNameMismatch, node.Name, node.NamespaceURI, name, ns));
                }
            }
            else
                w.WriteStartElement(name, ns);
        ...
        void WriteArray(object o, Type type) {
            ...
            if (type.IsArray) {
                ...
                if (typeof(ICollection).IsAssignableFrom(type)) {
                    w.WriteAttributeString("arrayType", Soap.Encoding, GetQualifiedName(typeName, typeNs) + "[" + ((ICollection)o).Count + "]");
                }
                else {
                    w.WriteAttributeString("arrayType", Soap.Encoding, GetQualifiedName(typeName, typeNs) + "[]");
                }
        ...
        protected void WritePotentiallyReferencingElement(string n, string ns, object o, Type ambientType, bool suppressReference, bool isNullable) {
            ...
            Type t = o.GetType();
            if (Convert.GetTypeCode(o) == TypeCode.Object && !(o is Guid) && !(o is XmlQualifiedName) &&   !(o is XmlNode[])) {
                if (suppressReference && !IsIdDefined(o)) {
    ...
    internal class XmlSerializationWriterCodeGen {
        ...
        public XmlSerializationWriterCodeGen(IndentedWriter writer, TypeScope[] scopes, string access, string className) : this(writer, scopes){
            this.className = className;
            this.access = access;
        }
        public XmlSerializationWriterCodeGen(IndentedWriter writer, TypeScope[] scopes) {
            this.writer = writer;
            this.scopes = scopes;
        ...
        void WriteElement(string source, ElementAccessor element, string arrayName, bool writeAccessor) {
            ...
            string ns = element.Form == XmlSchemaForm.Qualified ? (writeAccessor ? element.Namespace : element.Mapping.Namespace) : "";
            if (element.Mapping is ArrayMapping) {
                ArrayMapping mapping = (ArrayMapping)element.Mapping;
                if (mapping.IsSoap) {
                    writer.Write(!writeAccessor ? "WritePotentiallyReferencingElement(" : "WriteReferencingElement(");
                    WriteQuotedCSharpString(name);
                    writer.Write(", ");
                    WriteQuotedCSharpString(ns);
                    writer.Write(", ");
                ...
                if (mapping.IsSoap) {
                    if (mapping.TypeDesc.IsRoot || !writeAccessor) {
                        writer.Write("WritePotentiallyReferencingElement(");
                    }
                    else {
                        writer.Write("WriteReferencingElement(");
                    }
                    WriteQuotedCSharpString(name);
                    writer.Write(", ");
                    WriteQuotedCSharpString(ns);
                    ...
                    else if (mapping.TypeDesc.IsRoot) {
