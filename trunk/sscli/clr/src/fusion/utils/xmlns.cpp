    ...
    while (idx < cNumRecs) {
        if (aNodeInfo[idx]->dwType == XML_ATTRIBUTE) {
            if (aNodeInfo[idx]->ulLen == XML_NAMESPACE_TAG_LEN &&
                !StrCmpN(aNodeInfo[idx]->pwcText, XML_NAMESPACE_TAG, XML_NAMESPACE_TAG_LEN)) {
                ...
                continue;
                     ...
                     !StrCmpN(aNodeInfo[idx]->pwcText, XML_NAMESPACE_PREFIX_TAG, XML_NAMESPACE_PREFIX_TAG_LEN)) {
                ...
                if (!pos) {
                    ...
                    while (pos) {
                        ...
                        if (!StrCmp(pMapNodeCur->_pwzPrefix, pwzPrefix)) {
        ...
        if (!pos) {
            ...
            *ppwzQualified = WSTRDupDynamic(pwzAttribute);
            ...
            while (pos) {
                ...
                if (!StrCmp(pMapNode->_pwzPrefix, pwzPrefix)) {
