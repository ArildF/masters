...
namespace System.IO.IsolatedStorage {
	...
	public sealed class IsolatedStorageFile : IsolatedStorage, IDisposable
    {
        ...
        private static void InitGlobalsNonRoaming(IsolatedStorageScope scope)
        {
            ...
            String rndName = GetRandomDirectory(rootDir);
            if (rndName == null) {  // Create a random directory
                ...
                try {   // finally...
                    rndName = GetRandomDirectory(rootDir);  // try again with lock
                    if (rndName == null) {
                        byte[] key = new byte[15];
			Win32Native.Random(true, key , key.Length);
                        rndName = ToBase32StringSuitableForDirName(key);
        ...
        internal static string GetRandomDirectory(String root)
        {
            String[] nodes = GetFileDirectoryNames(root + "*", "*", false);
            for (int i=0; i<nodes.Length; ++i) {
                if (nodes[i].Length == 24)
                    return nodes[i]; // Get the first directory
            }
            return null;
        }
        internal static Mutex CreateMutexNotOwned(string pathName)
        {
            ...
            new BinaryWriter(ms).Write(pathName.ToUpper());
            ms.Position = 0;
            return new Mutex(false, GetHash(ms));           
        }        
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern IntPtr nOpen(String infoFile);
    ...
    internal class IsolatedStorageFileEnumerator : IEnumerator
    {
        ...
        private static char s_SepExternal = System.IO.Path.DirectorySeparatorChar;
