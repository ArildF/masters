...
{
protected:
    CSourceModule   *m_pModule;                 // NOTE:  this is NOT a ref'd pointer (unnecessary)
...
public:
    CInteriorNode (CSourceModule *pModule, BASENODE *pContainer) : m_pModule(pModule), m_pContainerNode(pContainer), m_pErrors(NULL), m_iRef(0) {}
...
{
public:
    CPrimaryInteriorNode (CSourceModule *pModule, BASENODE *pContainer) : CInteriorNode (pModule, pContainer) {}
    ~CPrimaryInteriorNode ();
...
public:
    CSecondaryInteriorNode (CSourceModule *pModule, ALLOCHOST *pHost, BASENODE *pContainer) : CInteriorNode (pModule, pContainer), m_heap(pHost) {}
    ~CSecondaryInteriorNode() {}
    static CSecondaryInteriorNode *CreateInstance (CSourceModule *pModule, ALLOCHOST *pHost, BASENODE *pContainer) { return new CSecondaryInteriorNode (pModule, pHost, pContainer); }
