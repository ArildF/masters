...
#if PLATFORM_WIN32
        this.ppTable["_win32"] = System.Environment.OSVersion.Platform.ToString().StartsWith("Win32");
        this.ppTable["_x86"] = true;
#else
	this.ppTable["_win32"] = false; // There are no win32-specific functions available on Rotor; always report false.
        this.ppTable["_x86"] = false;   // It is less likely we are on x86 with Rotor, and we cannot be sure either way.
#endif
      ...
      internal static bool IsVisibilitySpec(JSToken token){
        return JSToken.Package <= token && token <= JSToken.Protected;
      }
