...
namespace DefaultNamespace {
    ...
    public class SecDBEdit
    {
        ...
        internal static void Help( String which, String message )
        {
            ...
            sb.Append( "Microsoft (R) Shared Source CLI SecDBEdit\n" );
        ...
        internal static ArrayList LoadDatabase( String file )
        {
            ...
            int numEntries = indexReader.ReadInt32();
                        
            Index[] indices = new Index[numEntries];
            
            for (int i = 0; i < numEntries; ++i)
            {
                indices[i] = new Index();

                indices[i].pXml = indexReader.ReadInt32();
                indices[i].cXml = indexReader.ReadInt32();
                indices[i].pBinary = indexReader.ReadInt32();
                indices[i].cBinary = indexReader.ReadInt32();
            }     
            
            ArrayList list = new ArrayList( numEntries );
        ...
        internal static void SaveDatabase( String file, ArrayList list )
        {
            ...
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                if (entry.xml != null)
                {
                    indexWriter.Write( (int)dbStream.Position );
                    indexWriter.Write( entry.xml.Length );
                    dbStream.Write( entry.xml, 0, entry.xml.Length );
                }
                else
                {
                ...
                if (entry.binary != null)
                {
                    ...
                    dbStream.Write( entry.binary, 0, entry.binary.Length );
                }
                else
                {
                    // No xml for this entry (shouldn't happen, but we'll support), just write zeros
