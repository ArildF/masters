...
// We'll define an upper limit that allows multiplication by sizeof(WCHAR)
// but still remains positive, and allows some room for pad.
// Under normal circumstances, we should never get anywhere near this limit.
#define MAKE_MAX_LENGTH 0x3fffff00
...
#define MAKE_MULTIBYTE_FROMWIDE(ptrname, widestr, codepage) \
    ...
    WszWideCharToMultiByte(codepage, 0, widestr, -1, (LPSTR)__CQuickBytes##ptrname.Ptr(), __l##ptrname-1, NULL, NULL); \
...
#define MAKE_WIDEPTR_FROMANSI(ptrname, ansistr) \
    ...
    if (MultiByteToWideChar(CP_ACP, 0, ansistr, -1, ptrname, __l##ptrname) == 0) \
        memset( ptrname, 0, __l##ptrname*sizeof(WCHAR) );
...
#define MAKE_WIDEPTR_FROMUTF8N(ptrname, utf8str, n8chrs) \
    ...
    WszMultiByteToWideChar(CP_UTF8, 0, utf8str, n8chrs, ptrname, __l##ptrname);  \
...
#ifndef __GNUC__ // gcc does not allow overriding of the default operator new
...
extern "C" 
{
void    SplitPath(register const WCHAR *, WCHAR *, WCHAR *, WCHAR *, WCHAR *);
...
inline BOOL ContainsUnmappableANSIChars(const WCHAR * widestr)
{
    return FALSE;
}
...
{
public:
    friend class DebuggerRCThread; //need to get offsets for ReadMem,WriteMem
...
public:
    ...
    typename CClosedHash<T>::ELEMENTSTATUS Status(BYTE *p) {return static_cast<H*>(this)->Status((T*)p);}

    void SetStatus(BYTE *p, typename CClosedHash<T>::ELEMENTSTATUS s) {static_cast<H*>(this)->SetStatus((T*)p, s);}
...
public:
    void Init() { pNames = 0; }
    void Init(LPWSTR str) { pNames = 0; Insert(str); }
    void Destroy();
...
{
public:
    inline DWORD val(LPWSTR keyName, DWORD defaultVal=0)
    {
...
private:
    void init(LPWSTR keyName, DWORD defaultVal=0);
...
{
public:
    inline LPWSTR val(LPWSTR keyName)
    {
        // make sure that the memory was zero initialized
        _ASSERTE(m_inited == 0 || m_inited == 1);
...
public:
    ...
    inline void ensureInit(LPWSTR keyName)
    {
        // make sure that the memory was zero initialized
        _ASSERTE(m_inited == 0 || m_inited == 1);
...
public:
    ...
    UnlockedLoaderHeap(DWORD dwReserveBlockSize, 
                       ...
                       size_t *pGlobalPerfCounter_LoaderBytes = NULL,
...
public:
    ...
    void *AllocAlignedmem(size_t dwSize, DWORD alignment, BOOL bGrowHeap = TRUE)
    {
        ...
        // DWORD align
        dwRoomSize = (dwRoomSize + ALLOC_ALIGN_CONSTANT) & (~ALLOC_ALIGN_CONSTANT);
...
{
public:
    static void RegisterOutOfMemoryCallback (FnUtilCodeCallback pfn)
    {
