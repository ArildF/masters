...
#define SEMAPHORE_MAGIC   0xD539EA3F
		 ...
		 IN LPCWSTR lpName)
{
    ...
    pSemaphore->dwMagic = SEMAPHORE_MAGIC;
...
ReleaseSemaphoreExit:
	...
	ENTRY ("ReleaseSemaphore returns BOOL %d\n", bRet);
    return bRet;
}
...
IsValidSemaphoreObject(
      Semaphore *pSemaphore)
{
    return (pSemaphore->dwMagic == SEMAPHORE_MAGIC); 
}
