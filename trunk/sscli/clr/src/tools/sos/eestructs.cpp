...
{                                                                     \
    ...
    ULONG_PTR moffset;                                                \
    ULONG_PTR invalid;                                                \
    moffset = 0; /* member offset */                                  \
    invalid = static_cast<ULONG_PTR>(-1);                             \
    if (size() > 0)                                                   \
      {
 ...
 *
 * Parameters:
 * base     The address of the beginning of the structure
 * offset   The offset of the structure member
 *
 * Return value:
 *  If offset < 4096, base+offset.  Otherwise, offset.
 */
...
void EEJitManager::JitCode2MethodTokenAndOffset(DWORD_PTR ip, METHODTOKEN *pMethodToken, DWORD *pPCOffset)
{
    ...
    *pPCOffset = (DWORD_PTR)(ip - GetCodeBody(pCHdr));
}
...
#else
                PORTABILITY_ASSERT("InitRegDisplay is not implemented on this platform.");
#endif 
