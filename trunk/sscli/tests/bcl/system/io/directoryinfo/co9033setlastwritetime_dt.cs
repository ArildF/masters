...
public class Co9032set_LastWriteTime_dt
{
	...
	public bool runTest()
	{
		...
		{   		
			...
			try {
				...
				if((dir2.LastWriteTime - DateTime.Now.AddYears(1)).Seconds > 0) {
					iCountErrors++;
			...
			try {
				...
				if((dir2.LastWriteTime - DateTime.Now.AddMonths(1)).Seconds > 0) {
					iCountErrors++;
			...
			try {
				...
				if((dir2.LastWriteTime - DateTime.Now.AddDays(1)).Seconds > 0) {
					iCountErrors++;
	...
	{
		bool bResult = false;
		Co9032set_LastWriteTime_dt cbA = new Co9032set_LastWriteTime_dt();
		try {
			bResult = cbA.runTest();
