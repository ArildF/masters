...
static BOOL PROCGetProcessStatus( HANDLE hProcess, PROCESS_STATE *state,
                                  DWORD *exit_code);
static void PROCSuspendOtherThreads(void);
static void PROCCondemnOtherThreads(void);
...
static const char THLIST_MUTEXNAME[]="thlmutex-%d.mtx";

static const int MAX_MUTEXNAME = 100;
...
LPWSTR pAppDir;
THREADLIST *pGThreadList;

#define PROCESS_PELOADER_FILENAME  "clix"

#define PROCESS_MAGIC     0xA48CDF2
           ...
           OUT LPPROCESS_INFORMATION lpProcessInformation)
{
    ...
    switch (iRet)
    {
        ...
        case FILE_PE: /* PE/COFF file */
            ...
            if ( PAL_GetPALDirectoryW( WidePath,
                                      (MAX_PATH - (strlen(PROCESS_PELOADER_FILENAME)+1))))
            {
                if(! WideCharToMultiByte(CP_ACP, 0, WidePath, -1,
            ...
            {
                ASSERT("PAL_GetPALDirectoryW failed to return the"
                       "pal installation direcotry \n");
    ...
    lpProcess->dwMagic = PROCESS_MAGIC;
...
void PROCQueueDllMainCalls(void)
{
    ...
    thread_list = pGThreadList;
    while(NULL != thread_list)
    {
        if(thread_list->lpThread->dwThreadId != this_thread)
        {
            num_threads++;
        }
    ...
    while(NULL != thread_list)
    {
        if(thread_list->lpThread->dwThreadId != this_thread)
        {
            ...
            thread_array[curr_thread] = (THREAD *)
                                 HMGRLockHandle2(thread_list->lpThread->hThread, HOBJ_THREAD);
            curr_thread++;
        }
...
void PROCCleanupProcess(BOOL bTerminateUnconditionally)
{
    ...
    // Force-unload all DLLs and call their PROCESS_DETACH routines
    // unless we're terminating unconditionally.
    LOADFreeModules(bTerminateUnconditionally);

    /* release all mutexes held by this process */
    MutexAbandonAllMutexes();
    /* close all handles, to delete all temporary files and release all shared 
       memory resources */
    
    /* this will reduce the initialization count to zero; some functions will 
       detect this and use a code path that does not rely on PAL services that 
       we're about to shut down (example : SYNCLeaveCriticalSection) */
    PALShutdown();    

    /* release all handles. actually all we really need is to delete all 
       temporary files, release references to shared objects, and release all 
       shared memory owned only by this process */
    HMGRStopHandleManager();

    /* unregister this process from shared memory */
    SHMCleanup();
}
        ...
        OUT LPFILETIME lpUserTime)
{
    ...
    ENTRY("GetProcessTime(hProcess=%p, lpExitTime=%p, lpKernelTime=%p,"
          "lpUserTime=%p)\n",
        ...
        DWORD dwProcessId)
{
    ...
    process_info->dwMagic = PROCESS_MAGIC;
...
PROCCreateInitialProcess(LPWSTR lpwstrCmdLine, LPWSTR lpwstrFullPath)
{
    ...
    pCurrentProcess->dwMagic = PROCESS_MAGIC;
...
PROCCleanupInitialProcess(VOID)
{
    ...
    pThreadList = pGThreadList;
    if (pThreadList != NULL)
    {
        HANDLE hThread = pThreadList->lpThread->hThread;
        PROCRemoveThread(pThreadList->lpThread);
        CloseHandle(hThread);
    }
    else
    {
    ...
    THREAD *lpThread)
{
    ...
    /* allocate a threadList element to store the thread */
    lpThreadElement = (THREADLIST *) malloc(sizeof(THREADLIST));
    ...
    lpThreadElement->lpThread = lpThread;
    lpThreadElement->next = NULL;
    ...
    lpThreadList = pGThreadList;

    /* if it's the first thread added to the list */
    if (lpThreadList == NULL)
    {
        pGThreadList = lpThreadElement;
    }
    /* otherwise, add the thread to the beginning of the list */
    else
    {
        lpThreadElement->next = lpThreadList;
...
PROCRemoveThread(
    THREAD *lpThread)
{
    THREADLIST *curThread, *prevThread;
    ...
    curThread = prevThread = pGThreadList;
    ...
    if (curThread->lpThread == lpThread)
    {
    ...
    void)
{
    ...
    iterThread = pGThreadList;

    while (iterThread != NULL)
    {
        count++;
        iterThread = iterThread->next;
    }
...
isValidProcessObject(
      PROCESS *lpProcess)
{
    return (lpProcess->dwMagic == PROCESS_MAGIC);
}
...
Function:
  ...
  Calls TerminateThread on all threads in the process, except the current 
...
static void PROCSuspendOtherThreads(void)
{
    ...
    while(1)
    {
        ...
        while (NULL != pThreadList)
        {
            ...
            if (pThreadList->lpThread->dwThreadId != GetCurrentThreadId())
            {
                /* skip already-suspended threads */
                if(!pThreadList->lpThread->final_suspension)
                {
                    pThreadList->lpThread->final_suspension = TRUE;                    
                    /* lock the object so it remains valid after we unlock the 
                       process */
                    pThread = (THREAD *)HMGRLockHandle2(
                                                pThreadList->lpThread->hThread, HOBJ_THREAD);
                    break;
                }
            }
...
static void PROCCondemnOtherThreads(void)
{
    ...
    while (NULL != pThreadList)
    {
        ...
        if (pThreadList->lpThread->dwThreadId != GetCurrentThreadId())
        {
            pWaitState = SHMPTR_TO_PTR(pThreadList->lpThread->waitAwakened);
