...
#define HANDLE_TO_INDEX(h) ((((INT)(h))>>2)-1)
...
PAL-specific behavior :
    ...
    -dwOptions must be DUPLICATE_SAME_ACCESS
        ...
        IN DWORD dwOptions)
{
    ...
    {
        ASSERT("Can't duplicate handle : dwOptions is %#x instead of "
          "DUPLICATE_SAME_ACCESS (%#x).\n", dwOptions, DUPLICATE_SAME_ACCESS);
        SetLastError(ERROR_INVALID_PARAMETER);
        goto DuplicateExit;
    }
...
HANDLE HMGRGetHandle(HOBJSTRUCT *handle_data)
{
    ...
    if(handle_data->type<0 || handle_data->type>=HOBJ_LAST)
    {
    ...
    free_handle_list = (INT)handle_table[index].object;
...
{
    INT handle_index=HANDLE_TO_INDEX(handle);
...
void HMGRUnlockHandle(HANDLE handle, HOBJSTRUCT *handle_data)
{
    ...
    case 0:
        break;
    case -1:
        ASSERT("Object validation failed!\n", retval);
        SetLastError(ERROR_INVALID_HANDLE);
        break;
    case -2:
        ASSERT("Encountered Shared Memory corruption!!\n");
...
{
    /* the pseudo handle for current thread and current process are
       considered as special handles */
