...
static LPVOID FMTMSG_LoadLibrary( )
{
    ...
    if ( !PAL_GetPALDirectoryW( WidePath, MAX_PATH ) )
    {
        ERROR( "Unable to retrieve the path.\n" );
        goto error;
    }

    if ( WideCharToMultiByte( CP_ACP, 0, WidePath, -1, PathAndFileName,
                                MAX_PATH, NULL, 0 ) == 0 )
    {
        ERROR( "Unable to convert the path to ANSI.\n" );
        goto error;
    }

#ifdef __APPLE__
#define ROTOR_PALRT "librotor_palrt.dylib"
#else
#define ROTOR_PALRT "librotor_palrt.so"
#endif
