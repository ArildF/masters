...
MoveFileExW(
        ...
        IN DWORD dwFlags)
{
    int   src_size, dest_size, result;
    char  source[MAX_PATH];
    char  dest[MAX_PATH];
    BOOL  bRet = TRUE;
    DWORD dwLastError = 0;

    ENTRY("MoveFileExW(lpExistingFileName=%p (%S), lpNewFileName=%p (%S), "
          "dwFlags=%#x)\n",
          lpExistingFileName?lpExistingFileName:W16_NULLSTRING,
          lpExistingFileName?lpExistingFileName:W16_NULLSTRING,
          lpNewFileName?lpNewFileName:W16_NULLSTRING,
    ...
    src_size = WideCharToMultiByte( CP_ACP, 0, lpExistingFileName, -1, source,
                                    MAX_PATH, NULL, NULL );
    if( src_size == 0 )
    {
        dwLastError = GetLastError();
    ...
    dest_size = WideCharToMultiByte( CP_ACP, 0, lpNewFileName, -1, dest,
                                     MAX_PATH, NULL, NULL );
    if( dest_size == 0 )
    {
        dwLastError = GetLastError();
    ...
    {
        bRet = DeleteFileW( lpNewFileName );
    ...
    if ( result < 0 )
    {
        ...
        case EXDEV: /* we tried to link across devices */
            ...
            if ( dwFlags & MOVEFILE_COPY_ALLOWED )
            {
                ...
                bRet = CopyFileW( lpExistingFileName,
                          lpNewFileName,
                          bFailIfExists );
...
static int FILEDuplicateHandle( HANDLE handle, HOBJSTRUCT *handle_data)
{
    ...
    file *file_data = (file*)handle_data, *data;
    ...
    else
    {
        ...
        {
            // This is to handle fds without a filename, like pipes.
            data->unix_filename = NULL;
        }
...
FILEGetSHMFileLocks(char *filename)
{
    ...
    /* the file is never been locked before.*/
