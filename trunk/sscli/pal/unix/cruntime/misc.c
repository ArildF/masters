...
static pthread_key_t ctime_tlsKey = 0;

static pthread_key_t localtime_tlsKey = 0;
...
struct PAL_tm *
__cdecl
PAL_localtime(const time_t *clock)
{
    struct tm tmpResult;
    struct PAL_tm *result;
    int ret;

    ENTRY( "localtime( clock=%p )\n",clock );

    /*Initialize the structure ,make sure its not already
      allocated using the pthread_getspecific */
    if (NULL == (result = pthread_getspecific(localtime_tlsKey)))
    {
        result = (struct PAL_tm *)malloc(sizeof (struct PAL_tm));
        if(!result)
        {
            ERROR("Malloc failed\n");
            goto done;
        }
        if ((ret = pthread_setspecific(localtime_tlsKey, result)) != 0)
        {
            ASSERT("pthread_setspecific() failed error:%d (%s)\n",
                   ret, strerror(ret));
            free(result);
            result = NULL;
            goto done;
...
PAL_ctime( const time_t *clock )
{
    ...
    if (NULL == (Buf = pthread_getspecific(ctime_tlsKey)))
    {
        Buf = (char*)malloc(STR_TIME_SIZE * sizeof(char));
        if(!Buf)
        {
            ERROR("Malloc failed\n");
            goto done;
        }
        if ((ret = pthread_setspecific(ctime_tlsKey, Buf)) != 0)
        {
            ASSERT("pthread_setspecific() failed error:%d (%s)\n",
                   ret, strerror(ret));
            free(Buf);
...
MiscInitialize(void)
{
    ...
    /* allocate the TLS index for ctime Buffers */
    if ((ret = pthread_key_create(&ctime_tlsKey, NULL)) != 0)
    {
       return FALSE;
    }

    /* allocate the TLS index for localtime Buffers */
    if ((ret = pthread_key_create(&localtime_tlsKey, NULL)) != 0)
    {
       pthread_key_delete(ctime_tlsKey);        
       return FALSE;
    }
...
void MiscCleanup(void)
{
    ...
    /* free the index in TLS for char buffer */
    if((ret = pthread_key_delete(ctime_tlsKey)) != 0)
    {
        ERROR("pthread_key_delete() failed errno:%d (%s)\n",
               ret, strerror(ret));
    }

    /* free the index in TLS for tm structures */
    if((ret = pthread_key_delete(localtime_tlsKey)) != 0)
    {
        ERROR("pthread_key_delete() failed errno:%d (%s)\n",
               ret, strerror(ret));
    }
...
Termination function called from TerminateThread to free the
buffers allocated in PAL_localtime and PAL_ctime
--*/
void MiscFreeBuffers(void)
{
    char * Buf;
    struct PAL_tm *result;
    TRACE("Cleaning Misc Buffers...\n");

    /* free the ctimeBuffer associated with the main thread */
    if (NULL != (Buf = pthread_getspecific(ctime_tlsKey)))
    {
        free(Buf);
    }

    /* free the localtime buffer associated with the main thread */
    if (NULL != (result = pthread_getspecific(localtime_tlsKey)))
    {
        free(result);
    }
