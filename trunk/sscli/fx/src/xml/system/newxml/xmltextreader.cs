    ...
    public class XmlTextReader : XmlReader, IXmlLineInfo {
       ...
       // protected constructor, users should not be using this
        ...
        public XmlTextReader(String url, Stream input, XmlNameTable nt) : this(nt) {
            ...
            _BaseURI = (url==null? String.Empty: NameTable.Add(url));
        }
        ...
        private void InitFragmentReader(XmlNodeType fragType, XmlParserContext context) {
            ...
            _BaseURI                = (null == context ? String.Empty : NameTable.Add(context.BaseURI));
            ...
            switch (fragType) {
                ...
                default:
                {
                    throw new XmlException(Res.Xml_PartialContentNodeTypeNotSupported);
                }
            }
        ...
        public XmlTextReader(String url, XmlNameTable nt): this(nt) {
            ...
            _BaseURI = _NameTable.Add(XmlUrlResolver.UnEscape(uri.ToString()));
        }
        private void CreateScanner() {
            Stream stream = null;
            XmlResolver myResolver = (_XmlResolver != null ? _XmlResolver : new XmlUrlResolver());
            Uri uri = myResolver.ResolveUri(null, _BaseURI);
            try {
        ...
        private    void ParseXmlDecl() {
            ...
            while ((token = _Scanner.ScanMarkup()) != XmlToken.ENDPI) {
                ...
                if (_Scanner.IsToken(s_Version)) {
                    if(actualStart != -1)
                        _StringBuilder.Append(_Scanner.InternalBuffer, actualStart, _Scanner.CurrentPos-actualStart);
                    else
                        _StringBuilder.Append(_Scanner.InternalBuffer, _Scanner.TextOffset, _Scanner.TextLength);
                    if (0 != i) {
                ...
                actualStart = _Scanner.CurrentPos;
                ...
                if (token == XmlToken.TEXT) {
                    ...
                    _StringBuilder.Append(text, 0, text.Length);
                    switch (i) {
                        case 1:
                            //
       ...
       // parse from root
        ...
        public override bool Read() {
            ...
            while (!_Eof) {
                    ...
                    switch (_NextFunction) {
                        ...
                        case _FragmentXmlDeclParser:
                            ...
                            if (XmlToken.EOF != token) {
                                throw new XmlException(Res.Xml_InvalidPartialContentData);
                            }
                            break;
                ...
                if (_ScannerEof) {
                    ...
                    //
                    if (_RootCount < 1) {
                        throw new XmlException(Res.Xml_MissingRoot);
                    }
        ...
        /// <include file='doc\XmlTextReader.uex' path='docs/doc[@for="XmlTextReader.ReadInnerXml"]/*' />
        /// <devdoc>
        ...
        public override String ReadInnerXml() {
            ...
            if ( _NextFunction == _FinishReadChars) {
                Read();
                return String.Empty;
            }

            if (this.NodeType == XmlNodeType.Attribute) {
                return _Fields[_ReadCount].IgnoreValue;
            }
            ...
            int startPos = _Scanner.CurrentPos;
            _BufferConsistency = true;
            _Scanner.ReadBufferConsistency = _Scanner.StartPos;
            SkipElementScope();
            String str = _Scanner.GetText(startPos, _CurrentEndElementStart - startPos);
            Read();
            _BufferConsistency = false;
            _Scanner.ReadBufferConsistency = -1;
            return str;
        }
        ...
        /// <include file='doc\XmlTextReader.uex' path='docs/doc[@for="XmlTextReader.ReadOuterXml"]/*' />
        /// <devdoc>
        ///    <para>[To be supplied.]</para>
        /// </devdoc>
        public override String ReadOuterXml() {
            ...
            int startPos = _CurrentElementStart;
            _Scanner.ReadBufferConsistency = startPos;
            String str;

            if (this.NodeType == XmlNodeType.Attribute) {

                return this.Name+'='+ _Fields[_ReadCount].QuoteChar + _Fields[_ReadCount].IgnoreValue +_Fields[_ReadCount].QuoteChar ;
            }
            ...
            if (_ElementToken.IsEmpty) {
                str = _Scanner.GetText(startPos, _CurrentEndElementEnd - startPos);
                Read();
            }
            else  {
                _BufferConsistency = true;
        ...
        /// <include file='doc\XmlTextReader.uex' path='docs/doc[@for="XmlTextReader.ReadString"]/*' />
        /// <devdoc>
        ...
        public override string ReadString() {
            ...
            else if ((NodeType == XmlNodeType.Element) && !_ElementToken.IsEmpty) {
                if (! Read())
                    throw new InvalidOperationException(Res.GetString(Res.Xml_InvalidOperation));
            }
        ...
        internal XmlTextReader(XmlScanner scanner, XmlNameTable nt, XmlNamespaceManager nsMgr, XmlNodeType nodeType, int depth, Encoding encoding, String baseURI, bool external, bool standAlone) : this(nt) {
            _Scanner = scanner;
            _NsMgr  = nsMgr;
            _BaseURI = baseURI==null? String.Empty : _NameTable.Add(XmlUrlResolver.UnEscape(baseURI));
            _RootCount = 1;
            _NextState = 2;
            _ElementDepth = -1;
        ...
        internal String AttributeValue {
            set { _Fields[_ReadCount].ExpandValue = value; }
        }
        ...
        private bool ReturnWhitespaceToken(StringBuilder stringBuilder) {
            bool returnWS = false;
            ...
            _WhitespaceToken.SetValue(stringBuilder);
