...
{
    HRESULT hr = S_OK;
    const int cElems = ASM_MANIFEST_IMPORT_DEFAULT_ARRAY_SIZE;
...
*/
    ...
    if ((hr = FAILED(CreateAssemblyNameObjectFromMetaData(ppName, szAssemblyName, &amd, NULL))))
        goto exit;
...
HRESULT CAssemblyManifestImport::CopyModuleRefs(IMetaDataAssemblyImport *pMDImport)
{
    ...
    ccPath = (DWORD)(pszName - _szManifestFilePath);
