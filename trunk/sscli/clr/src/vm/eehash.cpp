...
EEClassHashEntry_t *EEClassHashTable::FindNextNestedClass(LPCUTF8 pszFullyQualifiedName, HashDatum *pData, EEClassHashEntry_t *pBucket)
{
    ...
    {
        memcpy(szNamespace, pszFullyQualifiedName, p - pszFullyQualifiedName);
        szNamespace[ p - pszFullyQualifiedName ] = '\0';
        p++;
    }
    else
    {
...
{
#ifdef _DEBUG
    m_dwDebugMemory += (DWORD) ((SIZEOF_EEHASH_ENTRY + sizeof(mdScope) + sizeof(mdTypeDef)));
#endif
...
{
    return (DWORD)((DWORD_PTR)sc ^ (DWORD_PTR)cl);
}
