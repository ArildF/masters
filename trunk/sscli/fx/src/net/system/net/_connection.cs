...
namespace System.Net {
    ...
    internal class Connection {
        ...
        /*++
            ...
            Returns:
                    HttpAbortDelegate - delegate that can cancel request

        --*/

        public HttpAbortDelegate SubmitRequest(HttpWebRequest Request) {
        ...
        private void StartConnectionCallback(object state, bool wasSignalled) {
            ...
            try {
                ...
                if ((httpWebRequest.Address.Scheme != Uri.UriSchemeHttps) || !m_Server.InternalProxyServicePoint) {
                    ...
                    m_AbortSocket = socket;
                    ws = m_Server.ConnectSocket(socket);
    ...
    report_error:
            ...
            HandleError(ws, ConnectionFailureGroup.Connect, ref returnResult);
            ConnectionReturnResult.SetResponses(returnResult);
        ...
        internal bool ReadStartNextRequest(bool PostReceive, ref ConnectionReturnResult returnResult) {
            ...
            if (NextRequest != null) {
                ...
                StartRequest(NextRequest, PostReceive);
            }
            else {
                Monitor.Exit(this);
                ...
                if (PostReceive) {
                    GlobalLog.Print("Connection#" + ValidationHelper.HashString(this) + "::ReadStartNextRequest() ThreadPool.QueueUserWorkItem(m_PostReceiveDelegate, this)");
                    ThreadPool.QueueUserWorkItem(m_PostReceiveDelegate, this);
                }
            }
        ...
        ParseStatusLine(
            ...
            GlobalLog.Assert((statusLineLength - index) >= 0,
                "(statusLineLength - index) < 0",
            ...
            while ((index < statusLineLength) && (statusLine[index] != '\r') && (statusLine[index] != '\n')) {
                ...
                switch (statusState) {
                    ...
                    case StatusCodeNumber:
                        ...
                        if (Char.IsDigit((char)statusLine[index])) {
                            int val = statusLine[index] - '0';
                        ...
                        else if (!Char.IsWhiteSpace((char) statusLine[index])) {
                    ...
                    case AfterStatusCode:
                        if (statusLine[index] != ' ') {
                            lastUnSpaceIndex = index;

                            if (startIndexStatusDescription == -1) {
                                startIndexStatusDescription = index;
                            }
                        }
            ...
            while ((index < statusLineLength)
                   && ((statusLine[index] == '\r') || (statusLine[index] == ' '))) {
                ++index;
                ++bytesParsed;
            }
...
quit:
        ...
        private static void ReadCallback(IAsyncResult asyncResult) {
            ...
            try {
                ...
                if (parseStatus == DataParseStatus.Invalid) {
                    ...
                    GlobalLog.Print("Connection#" + ValidationHelper.HashString(thisConnection) + "::ReadCallback() parseStatus == DataParseStatus.Invalid");
                    goto done;
                }
                ...
                //
                if (thisConnection.m_ReadState==ReadState.Headers && thisConnection.m_BytesRead==thisConnection.m_ReadBuffer.Length) {
        ...
        /*++
            ...
            This method is called by a stream interface to complete
            the writing.  It handles this by retreiving result of the
            operation.
        ...
        private bool TunnelThroughProxy(Uri proxy, HttpWebRequest originalRequest, out Socket socket) {
            ...
            try {
                ...
                connectRequest.Credentials = originalRequest.Proxy.Credentials;
                connectRequest.Proxy = GlobalProxySelection.GetEmptyWebProxy();
        ...
        internal void Dump() {
            ...
            if (Transport!=null && Transport.m_StreamSocket!=null) {
