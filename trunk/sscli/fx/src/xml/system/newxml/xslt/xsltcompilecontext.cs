...
namespace System.Xml.Xsl {
    ...
    internal class XsltCompileContext : XsltContext {
        ...
        private MethodInfo GetExtentionMethod(string ns, string name, XPathResultType[] argTypes, out object extension) {
            extension = this.processor.GetScriptObject(ns);
            if (extension != null) {
                return FindBestMethod(extension.GetType().GetMethods(bindingFlags), /*ignoreCase:*/true, /*publicOnly:*/false, name, argTypes);
            }

            extension = this.processor.GetExtensionObject(ns);
            if(extension != null) {
                return FindBestMethod(extension.GetType().GetMethods(bindingFlags), /*ignoreCase:*/false, /*publicOnly:*/true, name, argTypes);
            }
        ...
        private Uri ComposeUri(string thisUri, string baseUri) {
            ...
            Uri uriBase = null; {
                if (! object.ReferenceEquals(baseUri, FuncDocument.VoidBaseUri)) {
        ...
        internal bool FindKeyMatch(XmlQualifiedName name, String value, ArrayList ResultList, XPathNavigator context) {
            ArrayList      KeyList = this.processor.RootAction.KeyList;
            ...
            for (int i=0; i< KeyList.Count; i++) {
                ...
                if (key.Name == name) {
                    ...
                    while( sel.MoveNext() ) {
                        EvaluateKey( sel, matchExpr, useExpr, ResultList, value);
                        XPathNodeIterator attrsel = sel.Current.Select("attribute::*"); 
                        while ( attrsel.MoveNext() ) {
                            EvaluateKey( attrsel, matchExpr, useExpr, ResultList , value);
                        }
                    }
                }
            }
        ...
        internal bool KeyMatches(String value, XPathExpression query, XPathNavigator nav) {
        ...
        private void EvaluateKey(XPathNodeIterator sel, XPathExpression matchExpr,
                                 XPathExpression useExpr, ArrayList ResultList, String value) {
            ...
            if ( nav.Matches( matchExpr ) ) {
                ...
                if (useExpr.ReturnType != XPathResultType.NodeSet) {
                    ...
                    if (KeyMatches(value, useExpr, nav) ) {
                        AddResult( ResultList, nav );
                    }
                }
            }
        ...
        private XPathNodeIterator Current() {
            XPathNavigator nav = this.processor.Context;
            if (nav != null) {
        ...
        // static helper methods:
            ...
            public static double ToNumber(object argument) {
                XPathNodeIterator it = argument as XPathNodeIterator;
                return XmlConvert.ToXPathDouble(it != null ? IteratorToString(it) : argument);
            }

            private static object ToNumeric(object argument, TypeCode typeCode) {
                try {
                    XPathNodeIterator it = argument as XPathNodeIterator;
                    return Convert.ChangeType(it != null ? IteratorToString(it) : argument, typeCode);
                }
                catch {
                    return double.NaN;
                }        
            ...
            public static object ConvertToXPathType(object val, XPathResultType xt, TypeCode typeCode) {
                ...
                case XPathResultType.String    : 
                    if(val is XPathNavigator) return ToNavigator(val);
                    return ToString(val);
...
//                case XPathResultType.Navigator : return ToNavigator(val);
        ...
        private class FuncDocument : XsltFunctionImpl {
            ...
            public override object Invoke(XsltContext xsltContext, object[] args, XPathNavigator docContext) {
                ...
                if (args.Length == 2) {
                    ...
                    if (it.MoveNext()){
                        ...
                        baseUri = VoidBaseUri; // call to Document will fail if args[0] is reletive.
                    }
                }
                try {
            ...
            static public string VoidBaseUri = "$$$VoidBaseUri$$$";
        } 
