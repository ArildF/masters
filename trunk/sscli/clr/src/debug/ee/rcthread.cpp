...
HRESULT DebuggerRCThread::SetupRuntimeOffsets(DebuggerIPCControlBlock *pDCB)
{
    ...
    pRO->m_offRgData = offsetof(DebuggerPatchTable, m_pcEntries);
    pRO->m_offCData = offsetof(DebuggerPatchTable, m_iEntries);
...
static LONG _debugFilter(LPEXCEPTION_POINTERS ep, PVOID pv)
{
    ...
    CorMessageBox(NULL, IDS_DEBUG_UNHANDLEDEXCEPTION_IPC, IDS_DEBUG_SERVICE_CAPTION,
...
void DebuggerRCThread::ThreadProc(void)
{
    ...
    if (m_rgDCB[IPC_TARGET_OUTOFPROC]->m_temporaryHelperThreadId != 0)
    {
        ...
        DWORD ret =
#endif
        WaitForSingleObject(m_helperThreadCanGoEvent, INFINITE);
...
void DebuggerRCThread::MainLoop(bool temporaryHelp)
{
    ...
    while (m_run)
    {
        ...
        if (waitCount == DRCT_COUNT_INITIAL && m_rgDCB[IPC_TARGET_OUTOFPROC]->m_rightSideProcessHandle != NULL)
        {
            ...
            if(m_rgDCB[IPC_TARGET_OUTOFPROC]->m_remoteRightSideProcessHandle)
            {
                m_rgDCB[IPC_TARGET_OUTOFPROC]->m_rightSideProcessHandle = 
                    PAL_RemoteHandleToLocal(m_rgDCB[IPC_TARGET_OUTOFPROC]->m_remoteRightSideProcessHandle);
    
                if (m_rgDCB[IPC_TARGET_OUTOFPROC]->m_rightSideProcessHandle == INVALID_HANDLE_VALUE)
                {
                    m_rgDCB[IPC_TARGET_OUTOFPROC]->m_rightSideProcessHandle = NULL;
                }
                else
                {
                ...
                m_rgDCB[IPC_TARGET_OUTOFPROC]->m_remoteRightSideProcessHandle = NULL;
            }
        }
