...
GetNumInfo(LCID lcid, unsigned long dwFlags, NUMINFO ** ppnuminfo)
{
    ...
    *ppnuminfo = NULL;
    ...
    // CONSIDER: look at current thread cache to see if we
    // already have the one we're looking for.
    // Since it uses TLS, it may actually be more overhead
    // than looking in our static cache.
...
CNumInfo::Init(LCID lcid, unsigned long dwFlags)
{
    ...
    // If the thousands and decimal separators are set the same, then we are
    // screwed.  Don't strip off any thousand separators.
