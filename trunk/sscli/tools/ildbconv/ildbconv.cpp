...
// Globals
HINSTANCE  g_PalRtDll = NULL; // PalRt Dll handle
                             ...
                             void     **ppv)
{
    ...
    dllGetClassObject = (DLLGETCLASSOBJECT*)GetProcAddress(g_PalRtDll, "PAL_CoCreateInstance");

    if (dllGetClassObject == NULL) {
        goto Win32Error;
    }

    hr = (*dllGetClassObject)(rclsid, riid, (void**)ppv);
    return hr;
...
)
{
    ...
    if (cNameSpaces)
    {
        ...
        _ASSERT(cNameSpacesDummy == cNameSpaces);
    ...
    if (cChildScopes)
    {
        ...
        _ASSERT(cDummyChildren == cChildScopes);
...
HRESULT ConvertFromIldbToPdb(char *szExeFileName)
{
    ...
    IfFailGo(CoCreateInstance(CLSID_CorSymWriter,
                              NULL,
                              CLSCTX_INPROC_SERVER,
    ...
    IfFailGo(PAL_CoCreateInstance(CLSID_CorSymBinder, IID_ISymUnmanagedBinder, (void **)&pSymBinder));
...
HRESULT ConvertFromPdbToIldb(char *szExeFileName)
{
    ...
    IfFailGo(CoCreateInstance(CLSID_CorSymBinder,
                              NULL,
                              CLSCTX_INPROC_SERVER,
    ...
    IfFailGo(PAL_CoCreateInstance(CLSID_CorSymWriter, IID_ISymUnmanagedWriter, (void **)&pSymWriter));
...
Error:
    ...
    if (FAILED(hr))
    {
        ...
        else
        {
            ...
            wszDescription[0] = '\0';
            FormatMessageW(FORMAT_MESSAGE_MAX_WIDTH_MASK | FORMAT_MESSAGE_FROM_SYSTEM, NULL, 
            hr, GetSystemDefaultLangID(), wszDescription, sizeof(wszDescription) / sizeof(WCHAR), NULL);
