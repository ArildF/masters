...
void NestedFunc1 (void) 
{
    ...
    PAL_EXCEPT_FILTER_EX(filt_1, ContSearchFilter, (LPVOID)&nValidator)
    {
        Fail("PAL_EXCEPT_FILTER: ERROR -> Something weird is going on."
...
int __cdecl main(int argc, char *argv[])
{
    ...
    */
    ...
    PAL_TRY 
    {
        ...
        PAL_EXCEPT_FILTER_EX(Filt_1, ExecExeptionFilter, (LPVOID)&nValidator)
        {
            if (!bTry1)
            {
                Fail("PAL_EXCEPT_FILTER: ERROR -> Something weird is going on."
