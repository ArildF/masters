...
// 
//   
//    Copyright (c) 2002 Microsoft Corporation and/or its suppliers.  All rights reserved.
//   
...
void* gmallocHeap::wsbrk (long size)
{
    ...
    if (size > 0)
    {
        ...
        if (gAddressBase == 0)
        {
            ...
            gNextAddress = gAddressBase = (BYTE*)VirtualAlloc (NULL, gAllocatedSize,
                                            MEM_RESERVE, PAGE_NOACCESS);
            WS_PERF_LOG_PAGE_RANGE((void*)0x2, gAddressBase, gAddressBase + gAllocatedSize - OS_PAGE_SIZE, gAllocatedSize);
            ...
            for(;;)
            {
...
#endif //MAP_VIEW
				{
					gAddressBase = gNextAddress =
                        (BYTE*)VirtualAlloc (new_address, new_size,
													mem_reserve, PAGE_NOACCESS);
        ...
        if ((DWORD_PTR)(size + gNextAddress) > AlignPage (gNextAddress))
        {
            ...
            WS_PERF_LOG_PAGE_RANGE((void*)0x2, res, (unsigned char *)res + (DWORD_PTR)(size + gNextAddress - AlignPage (gNextAddress)) - OS_PAGE_SIZE, (SIZE_T)(size + gNextAddress - AlignPage (gNextAddress)));
        }
        tmp = gNextAddress;
    ...
    else if (size < 0)
    {
        ...
        {
            VirtualFree (alignedGoal, gNextAddress - alignedGoal,
                         MEM_DECOMMIT);
        ...
        {
            VirtualFree (gAddressBase, gNextAddress - gAddressBase,
                         MEM_DECOMMIT);
            gNextAddress = gAddressBase;
            return (void*)-1;
        }
