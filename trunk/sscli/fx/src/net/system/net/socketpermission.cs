    ...
    public class EndpointPermission {
        ...
        internal bool IsDns {
            get {
                ...
                return IsNotAllDigits(hostname);
            }
        }

        //
        // Return true only if the input contains some symbol
        // other than digits and dots.
        //
        // Inputs like 256.0.0.1 or 1.1.1.1.1 produce false result.
        // The basic idea is to pass such strings to DNS as to
        // final decision maker.
        //
        private static bool IsNotAllDigits(string name) {
            int  start =0;
            int  end = name.Length;
            while (start < end) {
                if (!Char.IsDigit(name[start]) && name[start] != '.') {
                    return true;
                }
                ++start;
            }
            return false; //only digits and dots
        }
...
*/
    internal static bool IsValidEndPointName(string name) {
        if (name == null) {
            return false;
        }
