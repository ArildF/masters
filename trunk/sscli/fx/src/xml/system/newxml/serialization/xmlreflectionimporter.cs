...
namespace System.Xml.Serialization {
    ...
    public class XmlReflectionImporter {
        ...
        public XmlTypeMapping ImportTypeMapping(Type type, XmlRootAttribute root, string defaultNamespace) {
            try {
                XmlTypeMapping xmlMapping = new XmlTypeMapping(typeScope, ImportElement(modelScope.GetTypeModel(type), root, defaultNamespace, XmlSchemaForm.Qualified));
                xmlMapping.GenerateSerializer = true;
                return xmlMapping;
            }
            catch (Exception e) {
                throw ReflectionException(type.FullName, e);
            }
        }
        ...
        void ImportAccessorMapping(MemberMapping accessor, FieldModel model, XmlAttributes a, string ns) {
            ImportAccessorMapping(accessor, model, a, ns, XmlSchemaForm.None, null);
        }
        ...
        void CheckAmbiguousChoice(XmlAttributes a, Type accessorType, string accessorName) {
            ...
            if (items != null && items.Count >= 2) {
                NameTable choiceTypes = new NameTable();

                for (int i = 0; i < items.Count; i++) {
                    Type type = items[i].Type == null ? accessorType : items[i].Type;
                    string ns = items[i].NestingLevel.ToString();
                    XmlArrayItemAttribute item = (XmlArrayItemAttribute)choiceTypes[type.FullName, ns];
                    if (item != null) {
                        ...
                        choiceTypes[type.FullName, ns] =  items[i];
                    }
                }
            }
        }
