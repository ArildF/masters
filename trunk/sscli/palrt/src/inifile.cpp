...
public:
    ...
    void AppendNewLine()
    {
        static const WCHAR newline[2] = { '\r', '\n' };
        Append(newline, sizeof(newline));
    }
                    ...
                    IN UINT cbParameterValueLength)
{
    ...
    if (bPerMachine) {
        b = PAL_GetMachineConfigurationDirectoryW(ConfigFileName, _MAX_PATH);
    } else {
        b = PAL_GetUserConfigurationDirectoryW(ConfigFileName, _MAX_PATH);
    }
