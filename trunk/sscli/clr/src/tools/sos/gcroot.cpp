...
void ProcessLargeHeap(size_t obj)
{
    ...
    if (dwLargeNPAddr == 0)
        dwLargeNPAddr = GetAddressOf (offset_class_gc_heap, 
              offset_member_gc_heap::large_np_objects);
    ...
    if (dwLargePAddr == 0)
        dwLargePAddr = GetAddressOf (offset_class_gc_heap, 
              offset_member_gc_heap::large_p_objects);
...
void InitGCRoot (size_t obj)
{
    ...
    {
	// use an address larger than 0x1000 for non-server builds, so
	// gcheap.Fill doesn't assert that the address is not less than
	// 0x1000.  All fields in the non-server build are static so the
	// address is irrelevant.
        DWORD_PTR dwAddr = 0x10000;
        heap.Fill (dwAddr);
        if (!CallStatus)
            return;
...
{
#ifdef PLATFORM_UNIX
    ExtOut("Finding GC Roots on stacks is not supported on PLATFORM_UNIX\n");
#else
 ...
 *
 *                     These are currently NOT general purpose.
                ...
                for (int j = 0; j < HANDLE_HANDLES_PER_BLOCK; j ++)
                {
                    if (segment.rgValue[start+j]) {
                        ...
                        if (segment.rgBlockType[i] == HNDTYPE_PINNED) {
                            ...
                            name = nameU;
#ifdef PLATFORM_UNIX
                        sprintf(&name[15], "%p", segment.firstHandle+(start+j)*HANDLE_SIZE);
#else
...
void FindGCRootOnHandleTables2(GCInfoCallback callback)
{
    ...
    static DWORD_PTR sHandleTableMapAddr = (DWORD_PTR)-1;
    if (sHandleTableMapAddr == (DWORD_PTR)-1)
    {
