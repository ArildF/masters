...
/* Macro to flip the bytes of a word */
...
// Endianness macros taken from clr/src/inc/palclr.h.
#ifdef BIGENDIAN
...
WCHAR* convert(char * aString) 
{
    ...
    wideBuffer = malloc(size*sizeof(WCHAR));
    if (wideBuffer == NULL)
    {
        Fail("ERROR: Unable to allocate memory!\n");
    }
    MultiByteToWideChar(CP_ACP,0,aString,-1,wideBuffer,size*sizeof(WCHAR));
    return wideBuffer;
}
