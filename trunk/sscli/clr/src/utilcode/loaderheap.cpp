...
UnlockedLoaderHeap::UnlockedLoaderHeap(DWORD dwReserveBlockSize, 
                                       ...
                                       size_t *pGlobalPerfCounter_LoaderBytes,
...
UnlockedLoaderHeap::~UnlockedLoaderHeap()
{
    ...
    if (m_pGlobalPerfCounter_LoaderBytes)
        *m_pGlobalPerfCounter_LoaderBytes = *m_pGlobalPerfCounter_LoaderBytes - (DWORD) m_dwTotalAlloc;
...
BOOL UnlockedLoaderHeap::ReservePages(size_t dwSizeToCommit, 
                                      ...
                                      BOOL fCanAlloc)
{
    ...
    void *pTemp = VirtualAlloc(pData, dwSizeToCommit, MEM_COMMIT, m_flProtect);
    if (pTemp == NULL)
    {
    ...
    if (m_pGlobalPerfCounter_LoaderBytes)
        *m_pGlobalPerfCounter_LoaderBytes = *m_pGlobalPerfCounter_LoaderBytes + (DWORD) dwSizeToCommit;
    if (m_pPrivatePerfCounter_LoaderBytes)
                                               ...
                                               BOOL fCanAlloc)
{
    ...
    size_t dwSizeToCommit = max(dwMinSize - memAvailable, m_dwCommitBlockSize);
         ...
         m_pPtrToEndOfCommittedRegion + dwSizeToCommit <= m_pEndReservedRegion)
    {
        ...
        void *pData = VirtualAlloc(m_pPtrToEndOfCommittedRegion, dwSizeToCommit, MEM_COMMIT, m_flProtect);
        _ASSERTE(pData != NULL);
        if (pData == NULL)
            return FALSE;
...
#endif
{
    ...
    _ASSERTE(((UINT_PTR)m_pAllocPtr & ALLOC_ALIGN_CONSTANT) == 0);
