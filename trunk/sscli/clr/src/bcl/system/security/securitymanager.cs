   ...
   sealed public class SecurityManager
    {
       ...
       static public bool CheckExecutionRights
        ...
        // Given a file name and access type returns 
        static private bool CheckFileAccess(String file, int Access)
        {
			FileIOPermissionAccess ac=FileIOPermissionAccess.NoAccess;
			if ((Access&0x3FF)==0x3FF) //FILE_ALL_ACCESS
				ac=FileIOPermissionAccess.AllAccess;
			else
			{
				if ((Access&0x1)==0x1) //FILE_READ_DATA
					ac|=FileIOPermissionAccess.Read;
				if ((Access&0x2)==0x2) //FILE_WRITE_DATA
					ac|=FileIOPermissionAccess.Write;
				if ((Access&0x4)==0x4) //FILE_APPEND_DATA
					ac|=FileIOPermissionAccess.Append;
					
			}
			bool res=true;
			new FileIOPermission(ac,file).Demand();
