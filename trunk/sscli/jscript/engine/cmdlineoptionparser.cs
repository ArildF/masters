    ...
    **************************************************************/
    public class CmdLineOptionParser{
      public static bool IsSimpleOption(string option, string prefix){
        return (String.Compare(option, prefix, true, CultureInfo.InvariantCulture) == 0) ? true : false;
      }

      public static string IsArgumentOption(string option, string prefix){
        int prefixLength = prefix.Length;
        if (option.Length < prefixLength || String.Compare(option, 0, prefix, 0, prefixLength, true, CultureInfo.InvariantCulture) != 0)
          return null;
      ...
      public static object IsBooleanOption(string option, string prefix){
        int prefixLength = prefix.Length;
        if (option.Length < prefix.Length || String.Compare(option, 0, prefix, 0, prefixLength, true, CultureInfo.InvariantCulture) != 0)
          return null;
        if (option.Length == prefixLength)
          return true;
