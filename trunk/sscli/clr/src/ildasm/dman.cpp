        ...
        if(nFiles)
        {
            ...
            for(ULONG ix = 0; ix < nFiles; ix++)
            {
                                                            ...
                                                            &pHashValue,&cbHashValue,&dwFlags)))
                {
					...
					if(VAL32(g_CORHeader->EntryPointToken) == rFileTok[ix])
					{
        ...
        if(nManRes)
        {
            ...
            for(ULONG ix = 0; ix < nManRes; ix++)
            {
                                                                ...
                                                                &dwFlags)))         // [OUT] Flags.
                {
					...
					if((tkImplementation == mdFileNil)&&(g_pFile!=NULL)) // embedded resource -- dump as .resources file
					{
						if(pRes == NULL)
						{
						...
						if(pRes)
						{
							...
							if(g_fOnUnicode) pF = _wfopen(wzName,L"wb");
							else 
							{
								memset(sz,0,L);
								WszWideCharToMultiByte(CP_ACP,0,wzName,-1,sz,L,NULL,NULL);
								pF = fopen(sz,"wb");
							}
							if(pF)
							{
...
IMetaDataAssemblyImport* GetAssemblyImport(void* GUICookie)
{
    ...
    PAL_EXCEPT(EXCEPTION_EXECUTE_HANDLER) // if exception, it's screwed
    {
