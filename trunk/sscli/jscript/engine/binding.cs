...
namespace Microsoft.JScript 
{
    ...
    public abstract class Binding : AST{
        ...
        if (this.members != null && this.members.Length > 0){
          ...
          if (this.members.Length == 1 && this.members[0].MemberType == MemberTypes.Method){
            ...
            if (dt == Typeob.GlobalObject || 
              ...
              return Globals.BuiltinFunctionFor(ob, meth);
      ...
      internal bool IsCompatibleWithDelegate(Type delegateType){
        ...
        foreach (MemberInfo mem in this.members){
          if (mem is MethodInfo){
            ...
            if (returnType == drtype && Class.ParametersMatch(meth.GetParameters(), dparams)){
              this.member = meth;
              this.isFullyResolved = true;
              return true;
            }
      ...
      private int PlaceValuesForHiddenParametersOnStack(ILGenerator il, MethodInfo meth, ParameterInfo[] pars){
        ...
        Object[] attrs = meth.GetCustomAttributes(Typeob.JSFunctionAttribute, false);
      ...
      internal override void ResolveCall(ASTList args, IReflect[] argIRs, bool constructor, bool brackets){
            ...
            if (constructor)
              ...
              if (meth != null && meth.IsSpecialName){ //Property getter.
                ...
                if (pars == null || pars.Length == 0){
                  //Have to guard against calling some parameterless properties directly
                  if ((meth is JSWrappedMethod && ((JSWrappedMethod)meth).GetWrappedObject() is GlobalObject) ||
                    (argIRs.Length > 0 || (!(meth is JSMethod) && Typeob.ScriptFunction.IsAssignableFrom(meth.ReturnType)))){
                    this.member = member = this.ResolveOtherKindOfCall(args, argIRs, constructor, brackets);
                    return;
                  }
        ...
        if (member is MethodBase){
          if (member.IsDefined(Typeob.JSFunctionAttribute, false) && !(this.defaultMember is PropertyInfo)){
            int hidden = 0;
            Object[] attrs = member.GetCustomAttributes(Typeob.JSFunctionAttribute, false);
            JSFunctionAttributeEnum attr = ((JSFunctionAttribute)attrs[0]).attributeValue;
            ...
            this.member = LateBinding.SelectMember(this.members);
            return;
          }else if (constructor && member is ConstructorInfo && !(member is JSConstructor) && typeof(Delegate).IsAssignableFrom(member.DeclaringType)){
      ...
      internal override Object ResolveCustomAttribute(ASTList args, IReflect[] argIRs, AST target){
        ...
        if (jscons != null){
          ...
          if (cons != null){
            ...
            if (typeof(System.Attribute).IsAssignableFrom(attrType)){
              Object[] usageAttrs = attrType.GetCustomAttributes(typeof(System.AttributeUsageAttribute), false);
      ...
      private MemberInfo ResolveOtherKindOfCall(ASTList argList, IReflect[] argIRs, bool constructor, bool brackets){
        ...
        if (member is Type){
          if (constructor){
            ...
            if (!brackets && argIRs.Length == 1){
              ...
              if (typeof(Delegate).IsAssignableFrom((Type)member) && this.isFullyResolved){
                Binding b = argList[0] as Binding;
                if (b == null || !b.IsCompatibleWithDelegate((Type)member)){
                  this.context.HandleError(JSError.InvalidCall);
                }
              }
              return member;
        ...
        if (member is FieldInfo && ((FieldInfo)member).IsLiteral){
          ...
          if ((val is ClassScope || val is Type)){
            if (constructor){
              ...
              if (!brackets && argIRs.Length == 1){
                Type ty = val as Type;
                if (ty != null && typeof(Delegate).IsAssignableFrom(ty) && this.isFullyResolved){
                  Binding b = argList[0] as Binding;
                  if (b == null || !b.IsCompatibleWithDelegate(ty)){
                    this.context.HandleError(JSError.InvalidCall);
                  }
                }
        ...
        if (!brackets && ((t != null && Typeob.ScriptFunction.IsAssignableFrom(t)) || ir is ScriptFunction)){
          this.defaultMember = member;
          if (t == null){
            this.defaultMemberReturnIR = ir.GetType(); //The appropriate subtype of ScriptFunction
            ...
            if (this.member != null){
              if (this.member.Name == "CreateInstance" && this.member.DeclaringType == typeof(ActiveXObjectConstructor))
                this.context.HandleError(JSError.ActiveXObjectNotSupportedOnRotor, false);
            }
            return this.member;
          }else{
        ...
        if (!brackets && ir is Type && (typeof(Delegate)).IsAssignableFrom((Type)ir)){
          this.defaultMember = member;
          this.defaultMemberReturnIR = ir;
      ...
      private void TranslateToIL(ILGenerator il, Type rtype, bool preSet, bool preSetPlusGet, bool calledFromDelete){
        if (this.member is FieldInfo){
          ...
          if (!preSet){
            ...
            if (tok is FieldInfo && !((FieldInfo)tok).IsLiteral)
              ...
              (new ConstantWrapper(field.GetValue(null), this.context)).TranslateToIL(il, rtype);
              return;
            }else
        ...
        if (this.member is MethodInfo){
          MethodInfo meth = this.member is JSMethod ? ((JSMethod)this.member).GetMethodInfo(compilerGlobals) : 
                           (this.member is JSMethodInfo ? ((JSMethodInfo)this.member).method : (MethodInfo)this.member);
          if (typeof(Delegate).IsAssignableFrom(rtype)){
            ...
            ConstructorInfo cons = rtype.GetConstructor(new Type[]{Typeob.Object, typeof(UIntPtr)});
            if (cons == null) cons = rtype.GetConstructor(new Type[]{Typeob.Object, typeof(IntPtr)});
      ...
      private Object TranslateToSpeculativeEarlyBindings(ILGenerator il, Type rtype, bool getObjectFromLateBindingInstance){
        ...
        for (int i = 0, n = members.count; i < n; i++){
          ...
          if (member is FieldInfo){
            ...
            if (getter is JSMethod) 
              getter = ((JSMethod)getter).GetMethodInfo(this.compilerGlobals);
            else if (getter is JSMethodInfo)
              getter = ((JSMethodInfo)getter).method;
            if (getter.IsVirtual && !getter.IsFinal && !t.IsSealed)
      ...
      private Object TranslateToSpeculativeEarlyBoundCalls(ILGenerator il, Type rtype, ASTList argList, bool construct, bool brackets){
        ...
        foreach (IReflect c in classes){
          ...
          try{
            MethodInfo meth = JSBinder.SelectMethod(members, this.argIRs); //Returns property getters as well
            if (meth != null){
