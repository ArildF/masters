...
int lpArguments_test1[EXCEPTION_MAXIMUM_PARAMETERS];
int nArguments_test1 = EXCEPTION_MAXIMUM_PARAMETERS;

int lpArguments_test2[EXCEPTION_MAXIMUM_PARAMETERS+1];
int nArguments_test2 = EXCEPTION_MAXIMUM_PARAMETERS+1;
...
LONG Filter_test1(EXCEPTION_POINTERS* ep, VOID *pnTestInt)
{
    ...
    if (ep->ExceptionRecord->NumberParameters != nArguments_test1)
    {
...
int __cdecl main(int argc, char *argv[])
{
    ...
    PAL_EXCEPT_FILTER_EX(TEST1, Filter_test1, (LPVOID)&nValidator)
    {
        if (!bTry)
        {
    ...
    PAL_EXCEPT_FILTER_EX(TEST2, Filter_test2, (LPVOID)&nValidator)
    {
        if (!bTry)
        {
            Fail("RaiseException: ERROR -> Something weird is going on."
