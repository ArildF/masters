    ...
    public sealed class Convert{
      ...
      internal static Object Coerce(Object value, Object type, bool explicitOK){
        ...
        if (ta != null){
          ...
          int rank = ta.rank;
          Type eTy = eIr is Type ? (Type)eIr : eIr is ClassScope ? ((ClassScope)eIr).GetBakedSuperType() : Typeob.Object;
        ...
        if (csc != null){
          if (csc.HasInstance(value))
            ...
            if (ed != null){
              ...
              if (ew != null)
                if (ew.type == csc) return value; else throw new JScriptException(JSError.TypeMismatch);
              return new EnumWrapper(Coerce(value, ed.baseType), null, csc);
            }
            if (value == null || value is DBNull) return null;
            throw new JScriptException(JSError.TypeMismatch);
          }
        }else if (!(type is Type))
          type = Convert.ToType((IReflect)type);
        else if (type == Typeob.Type && value is ClassScope)
          return value;
        ...
        try{ checked{
          ...
          switch (Convert.GetTypeCode(value, ic)){
            ...
            case TypeCode.Decimal:
              ...
              switch (target){
                case TypeCode.Boolean: return Convert.ToBoolean(Decimal.ToDouble(dec));
            ...
            case TypeCode.String:
              ...
              switch (target){
                ...
                case TypeCode.Single: try {return Single.Parse(str, CultureInfo.InvariantCulture);}catch(Exception){goto case TypeCode.Double;}
                case TypeCode.Int64: try {return Int64.Parse(str, CultureInfo.InvariantCulture);}catch(Exception){goto case TypeCode.Double;}
                case TypeCode.UInt64: try {return UInt64.Parse(str, CultureInfo.InvariantCulture);}catch(Exception){goto case TypeCode.Double;}
                case TypeCode.Decimal: try {return Decimal.Parse(str, CultureInfo.InvariantCulture);}catch(Exception){goto case TypeCode.Double;}
                case TypeCode.DateTime: 
                  try{
   ...
   /*String*/   { false, false, false, false,  false, false, false, false, false, false, false, false, false, false, false, false,  false,   false,   true}
      ...
      internal static int ToInt32(Object value, IConvertible ic){
        switch (Convert.GetTypeCode(value, ic)){
          ...
          case TypeCode.Single: return (int)ic.ToSingle(null);
          case TypeCode.Double:
          case TypeCode.Decimal: return (int)ic.ToDouble(null);
          case TypeCode.Object:
          case TypeCode.DateTime:
            Object pval = Convert.ToPrimitive(value, PreferredType.Number, ref ic);
            if (pval != value)
              ...
              return 0;
          case TypeCode.String: return (int)Convert.ToNumber(ic.ToString(null));
        }
        return 0; //should never get here.
      }
      ...
      internal static IReflect ToIReflect(Type t, VsaEngine engine){
        ...
        else if (t == typeof(EnumeratorObject))
        ...
        else if (t == typeof(EvalErrorObject))
        ...
        else if (t == typeof(RangeErrorObject))
          result = glob.originalRangeError.Construct(new Object[0]);
        else if (t == typeof(ReferenceErrorObject))
