    ...
    public class XmlSchemaImporter {
        ...
        public XmlMembersMapping ImportAnyType(XmlQualifiedName typeName, string elementName) {
            ...
            if (mapping.Members.Length != 1 || !mapping.Members[0].Accessor.Any)
                return null;
            mapping.Members[0].Elements[0].Name = elementName;
            ...
            if (schema != null) accessor.Form = schema.ElementFormDefault;
        ...
        ElementAccessor ImportElement(XmlQualifiedName name, Type desiredMappingType, Type baseType) {
            ...
            accessor = ImportElement(element, string.Empty, desiredMappingType, baseType, name.Namespace);
        ...
        ElementAccessor ImportElement(XmlSchemaElement element, string identifier, Type desiredMappingType, Type baseType, string ns) {
        ...
        TypeMapping ImportType(XmlQualifiedName name, Type desiredMappingType, Type baseType) {
            ...
            if (mapping != null) {
                if (!(desiredMappingType.IsAssignableFrom(mapping.GetType())))
                    throw new InvalidOperationException(Res.GetString(Res.XmlElementImportedTwice, name.Name, name.Namespace, mapping.GetType().Name, desiredMappingType.Name));
            }
            else {
        ...
        void ImportGroup(XmlSchemaParticle group, string identifier, CodeIdentifiers members, string ns, bool mixed) {
            if (group is XmlSchemaChoice)
                ImportChoiceGroup((XmlSchemaChoice)group, identifier, members, ns, false);
            else
                ImportGroupMembers(group, identifier, members, ns, false, mixed);
        }
        ...
        bool GatherGroupChoices(XmlSchemaParticle particle, ArrayList list, string identifier, string ns) {
            if (particle is XmlSchemaGroupRef) {
                XmlSchemaGroupRef refGroup = (XmlSchemaGroupRef)particle;
                if (!refGroup.RefName.IsEmpty)
                    return GatherGroupChoices(FindGroup(refGroup.RefName), list, identifier, refGroup.RefName.Namespace);
            }
            else if (particle is XmlSchemaGroupBase) {
                ...
                for (int i = 0; i < group.Items.Count; i++) {
                    ...
                    if (item is XmlSchemaGroupBase) {
                        if (GatherGroupChoices((XmlSchemaParticle)item, list, identifier, ns))
                            groupRepeats = true;
                    }
                    else if (item is XmlSchemaAny) {
                        list.Add(ImportAny((XmlSchemaAny)item, true));
                    }
                        ...
                        if (abstractElement != null) {
                            ...
                            for (int j = 0; j < elements.Length; j++) {
                                ...
                                list.Add(ImportElement(elements[j], identifier, typeof(TypeMapping), null, ns));
                            }
                        }
                        else {
                            if (element.IsMultipleOccurrence) groupRepeats = true;
                            list.Add(ImportElement(element, identifier, typeof(TypeMapping), null, ns));
                        }
                    }
                }
                return groupRepeats;
        ...
        void ImportGroupMembers(XmlSchemaParticle particle, string identifier, CodeIdentifiers members, string ns, bool groupRepeats, bool mixed) {

            if (particle is XmlSchemaGroupRef) {
                XmlSchemaGroupRef refGroup = (XmlSchemaGroupRef)particle;
                if (!refGroup.RefName.IsEmpty) {
                    ImportGroupMembers(FindGroup(refGroup.RefName).Particle, identifier, members, refGroup.RefName.Namespace, groupRepeats, mixed);
                }
            }
            else if (particle is XmlSchemaGroupBase) {
                bool any = false;
                ...
                for (int i = 0; i < group.Items.Count; i++) {
                    ...
                    if (item is XmlSchemaChoice)
                        ImportChoiceGroup((XmlSchemaChoice)item, identifier, members, ns, groupRepeats);
                    else if (item is XmlSchemaElement)
                        ImportElementMember((XmlSchemaElement)item, identifier, members, ns, groupRepeats);
                    else if (item is XmlSchemaAny) {
                        ImportAnyMember((XmlSchemaAny)item, identifier, members, ns, mixed);
                        any = true;
                    }
                    else if (item is XmlSchemaParticle)
                        ImportGroupMembers((XmlSchemaParticle)item, identifier, members, ns, groupRepeats, mixed);
                }
                if (mixed && !any) {
                    ImportTextMember(members, null);
                }
        ...
        ElementAccessor ImportArray(XmlSchemaElement element, string identifier, string ns, bool repeats) {
            ...
            if (element.Form == XmlSchemaForm.None) {
                XmlSchema schema = schemas[ns];
                if (schema != null) arrayAccessor.Form = schema.ElementFormDefault;
            }
            else
        ...
        void ImportAttributeGroupMembers(XmlSchemaAttributeGroupRef group, string identifier, CodeIdentifiers members, string ns) {
            if (!group.RefName.IsEmpty) {
                ImportAttributeGroupMembers(FindAttributeGroup(group.RefName), identifier, members, group.RefName.Namespace);
                return;
            }
        }
