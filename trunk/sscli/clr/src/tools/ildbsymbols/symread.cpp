...
SymReader::~SymReader()
{
    ...
    {
        free(m_DataPointers.m_pBytes);
        free(m_DataPointers.m_pConstants);
        free(m_DataPointers.m_pDocuments);
        free(m_DataPointers.m_pMethods);
        free(m_DataPointers.m_pScopes);
        free(m_DataPointers.m_pSequencePoints);
        free(m_DataPointers.m_pStringsBytes);
        free(m_DataPointers.m_pUsings);
        free(m_DataPointers.m_pVars);
    }
    ...
    )
{
    ...
    IfFailGo(pIStream->Read(pSignature, sizeof(ILDB_SIGNATURE_SIZE), &ulBytesRead));
    _ASSERTE(ulBytesRead == ILDB_SIGNATURE_SIZE);
    ...
    IfFailGo(pIStream->Read(&GuidVersion, sizeof(GUID), &ulBytesRead));
    _ASSERTE(ulBytesRead == sizeof(GUID));
    ...
    IfNullGo(m_pPDBInfo = (PDBInfo *)malloc(sizeof(PDBInfo)));

    memset(m_pPDBInfo, 0 , sizeof(PDBInfo));
    IfFailGo(pIStream->Read(m_pPDBInfo, sizeof(PDBInfo), &ulBytesRead));
    _ASSERTE(ulBytesRead == sizeof(PDBInfo));
    ...
    {
        IfNullGo(m_DataPointers.m_pConstants = (SymConstant *)malloc(m_pPDBInfo->m_CountOfConstants*sizeof(SymConstant)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pConstants, m_pPDBInfo->m_CountOfConstants*sizeof(SymConstant), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfConstants*sizeof(SymConstant));
    }

    if (m_pPDBInfo->m_CountOfMethods)
    {
        IfNullGo(m_DataPointers.m_pMethods = (SymMethodInfo *)malloc(m_pPDBInfo->m_CountOfMethods*sizeof(SymMethodInfo)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pMethods, m_pPDBInfo->m_CountOfMethods*sizeof(SymMethodInfo), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfMethods*sizeof(SymMethodInfo));
    }

    if (m_pPDBInfo->m_CountOfScopes)
    {
        IfNullGo(m_DataPointers.m_pScopes = (SymLexicalScope *)malloc(m_pPDBInfo->m_CountOfScopes*sizeof(SymLexicalScope)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pScopes, m_pPDBInfo->m_CountOfScopes*sizeof(SymLexicalScope), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfScopes*sizeof(SymLexicalScope));
    }

    if (m_pPDBInfo->m_CountOfVars)
    {
        IfNullGo(m_DataPointers.m_pVars = (SymVariable *)malloc(m_pPDBInfo->m_CountOfVars*sizeof(SymVariable)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pVars, m_pPDBInfo->m_CountOfVars*sizeof(SymVariable), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfVars*sizeof(SymVariable));
    }

    if (m_pPDBInfo->m_CountOfUsing)
    {
        IfNullGo(m_DataPointers.m_pUsings = (SymUsingNamespace *)malloc(m_pPDBInfo->m_CountOfUsing*sizeof(SymUsingNamespace)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pUsings, m_pPDBInfo->m_CountOfUsing*sizeof(SymUsingNamespace), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfUsing*sizeof(SymUsingNamespace));
    }

    if (m_pPDBInfo->m_CountOfSequencePoints)
    {
        IfNullGo(m_DataPointers.m_pSequencePoints = (SequencePoint *)malloc(m_pPDBInfo->m_CountOfSequencePoints*sizeof(SequencePoint)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pSequencePoints, m_pPDBInfo->m_CountOfSequencePoints*sizeof(SequencePoint), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfSequencePoints*sizeof(SequencePoint));
    }

    if (m_pPDBInfo->m_CountOfDocuments)
    {
        IfNullGo(m_DataPointers.m_pDocuments = (DocumentInfo *)malloc(m_pPDBInfo->m_CountOfDocuments*sizeof(DocumentInfo)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pDocuments, m_pPDBInfo->m_CountOfDocuments*sizeof(DocumentInfo), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfDocuments*sizeof(DocumentInfo));
    }

    if (m_pPDBInfo->m_CountOfBytes)
    {
        IfNullGo(m_DataPointers.m_pBytes = (BYTE *)malloc(m_pPDBInfo->m_CountOfBytes*sizeof(BYTE)));
        IfFailGo(pIStream->Read(m_DataPointers.m_pBytes, m_pPDBInfo->m_CountOfBytes*sizeof(BYTE), &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfBytes*sizeof(BYTE));
    }
    ...
    if (m_pPDBInfo->m_CountOfStringBytes)
    {
        ...
        IfNullGo(m_DataPointers.m_pStringsBytes = (BYTE *)malloc(m_pPDBInfo->m_CountOfStringBytes));
        IfFailGo(pIStream->Read(m_DataPointers.m_pStringsBytes, m_pPDBInfo->m_CountOfStringBytes, &ulBytesRead));
        _ASSERTE(ulBytesRead == m_pPDBInfo->m_CountOfStringBytes);
    ...
    const WCHAR* szsearchPath)
{
    ...
    pVersionInfo = (GUID *)CurrentOffset;
    CurrentOffset += sizeof(GUID);

    if (memcmp(pVersionInfo, &ILDB_VERSION_GUID, sizeof(GUID)))
    {
        IfFailGo(HrFromWin32(ERROR_INVALID_DATA));
    }
    ...
    *ppRetVal = NULL;
    ...
    for (i = 0; i < m_pPDBInfo->m_CountOfDocuments; i++)
    {
        ...
        cchName = (ULONG32) MultiByteToWideChar(CP_UTF8,
                                                ...
                                                0,
                                                NULL);
        wcsDocumentUrlAlloc = (WCHAR *)malloc(cchName * sizeof(WCHAR));
        ...
        if (wcsDocumentUrlAlloc)
        {
            free(wcsDocumentUrlAlloc);
    ...
    SymDocument **ppDocument)
{
    ...
    if (m_pDocs == NULL)
    {
        IfNullGo(m_pDocs = (SymDocument **)malloc(m_pPDBInfo->m_CountOfDocuments * sizeof(void *)));
    ...
    )
{
    ...
    if (fFound)
    {
        for (;point < m_pData->m_pMethods[m_MethodEntry].EndSequencePoints(); point++)
        {
            ...
            if (m_pData->m_pSequencePoints[point].Document() == DocumentEntry &&
                ((m_pData->m_pSequencePoints[point].StartLine() == CODE_WITH_NO_SOURCE) ||
