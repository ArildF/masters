...
// 
// ==--==
#ifndef __CORSVCPRIV_h__
#define __CORSVCPRIV_h__
...
#define SERVICE_MAPPED_MEMORY_NAME L"ROTORSvcEventQueue"
...
struct ServiceEventBlock
{
    ...
    void InitQueues()
    {
        ...
        for (int i = 0; i < (int) MAX_EVENTS; i++)
        {
    ...
    {
        int idx = (int) (pEvent - arrEvents);
        _ASSERTE(idx >= 0 && (unsigned int) idx < MAX_EVENTS);
