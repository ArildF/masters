...
if (uc($^O) ne "MSWIN32") {
    ...
    $CSCFLAGS = "/debug $ENV{CSC_COMPILE_FLAGS}" . $NoLogoOption;
    ...
    $JSCCOMPILER = $CLIXName . $ENV{TARGETCOMPLUS} . "/jsc.exe" .  $NoLogoOption;
    $ENV{'JSCCOMP'} = $ENV{TARGETCOMPLUS} . "/jsc.exe";
        ...
        for (my $index = 0; $index < @files; $index++) {
            ...
            my($base, $dir, $ext) = fileparse($file,'\..*' );
            {
                ...
                if(not $test->{CMDLINE}) {
                    ...
                    } elsif ($test->{PERLDRIVER}) {
                        $test->{CMDLINE} = "perl " . $base . ".pl " . $CLIXName . $OutputDirWithSeperator . $base . ".exe";
                ...
                if((uc($ext) eq ".IL") or (uc($ext) eq ".IFL") ) {
                    if($index eq 0) {
                        $cmdline = "ilasm /debug" . $NoLogoOption . " /output:" . $OutputDirWithSeperator . "$base.exe $file";
                    } else {
                        $cmdline = "ilasm /debug /dll" . $NoLogoOption . " /output:" . $OutputDirWithSeperator . "$base.dll $file";
                    }
                } elsif (uc($ext) eq ".CS") {
                    my $Resource = "";
                    if($test->{HASRESOURCE}) {
